...
import { ArrowLeft, Sparkles, Shield, Globe, FileText, Eye, Database, Share2, Lock, UserCheck, Cookie, Users, RefreshCw, Phone, BookOpen, Server, Key, Settings, ExternalLink, Cpu } from "lucide-react";
      ...
      <main className="relative z-10 py-16">
        <div className="container max-w-4xl mx-auto px-6">
          <div className="mb-12">
            ...
            <p className="text-gray-400">{lang === "en" ? "Updated Date: September 11, 2026" : "更新日期：2026年9月11日"}</p>
            <p className="text-gray-400">{lang === "en" ? "Effective Date: September 11, 2026" : "生效日期：2026年9月11日"}</p>
          </div>
          ...
          <div className="space-y-6 text-gray-300">
            ...
            <ReflectiveCard className="!rounded-xl">
              <div className="flex items-start gap-4 p-6">
                ...
                <div>
                  ...
                  <p className="leading-relaxed mt-3">{lang === "en" ? `Where an in-product account cancellation control is available, you may use it to submit a cancellation request. You may also contact ${APP_BRAND.contactEmail}. After reasonably verifying a valid request, we will delete or anonymize relevant data in accordance with applicable law and the retention terms in this Policy.` : `如产品内提供账户注销功能，您可通过该功能提交注销请求。您也可联系 ${APP_BRAND.contactEmail}。在合理核验有效请求后，我们将依照适用法律及本政策的保留条款删除或匿名化相关数据。`}</p>
                  <p className="leading-relaxed mt-3">{lang === "en" ? '3.4 TikTok Authorization Revocation and Disconnection: You may revoke Star Gaze\'s access through your TikTok account settings at any time. In TikTok Account Management, Disconnect revokes or disables the applicable credential and marks the local binding as disconnected; it does not by itself erase the retained account identity, publishing-task history, aggregate statistics, or authorization audit records. After both the independent comment and publishing authorizations have been disconnected, use Delete all ordinary TikTok data or submit a valid deletion request to erase the remaining ordinary-account data, subject only to limited retention required for security, fraud prevention, legal compliance, or dispute resolution.' : '3.4 TikTok 授权撤回与解绑：您可以随时通过 TikTok 账户设置撤回 Star Gaze 的访问授权。在 TikTok 账号管理中，“断开连接”会撤销或停用相应凭据，并将本地绑定标记为已断开；该操作本身不会清除保留的账号身份、发布任务历史、聚合统计或授权审计记录。分别断开评论授权和发布授权后，请使用“删除全部普通 TikTok 数据”或提交有效的数据删除请求，以清除剩余普通账号数据，但出于安全、防欺诈、法律合规或争议处理所必需的有限保留除外。'}</p>
                </div>
              </div>
            </ReflectiveCard>
            ...
            <ReflectiveCard className="!rounded-xl">
              <div className="flex items-start gap-4 p-6">
                ...
                <div>
                  ...
                  <p className="leading-relaxed"><strong>{lang === "en" ? "4.1 Security Measures:" : "4.1 安全措施："}</strong> {lang === "en" ? "We use technical and organizational safeguards, including encrypted transport, managed-infrastructure protections for stored data, access controls, and least-privilege restrictions for sensitive systems. Data in transit is protected using TLS 1.2 or above." : "我们采用技术和组织安全措施，包括加密传输、托管基础设施提供的静态数据保护、访问控制以及对敏感系统实施最小权限限制。传输中的数据使用 TLS 1.2 及以上协议保护。"}</p>
            ...
            <ReflectiveCard className="!rounded-xl">
              <div className="flex items-start gap-4 p-6">
                ...
                <div>
                  ...
                  <p className="leading-relaxed"><strong>{lang === "en" ? "5.1 Storage and Processing Locations:" : "5.1 存储与处理地点："}</strong> {lang === "en" ? "Primary persistent storage for account, authorization, publishing, comment, and statistics data is hosted in the United States. Star Gaze's public application and API services are hosted in China and may transiently process personal information and platform-related requests and data as necessary to provide authorization, publishing, comment management, diagnostics, support, and security functions. Relevant data may also be transmitted to TikTok and the service providers disclosed in this Policy. We apply safeguards including encrypted transport, access controls, least-privilege restrictions, and log redaction." : "账号、授权、发布、评论及统计数据的主要持久化存储位于美国。Star Gaze 的公开应用和 API 服务托管于中国，可能在提供授权、发布、评论管理、故障诊断、支持及安全保护功能所必需的范围内，临时处理个人信息以及平台相关请求和数据。相关数据还可能按功能需要传输至 TikTok 及本政策披露的服务提供商。我们采用加密传输、访问控制、最小权限和日志脱敏等保护措施。"}</p>
                  <p className="leading-relaxed mt-3"><strong>{lang === "en" ? "5.2 Storage Period:" : "5.2 存储期限："}</strong> {lang === "en" ? "We retain personal information only as necessary for the service and security, fraud-prevention, legal, accounting and dispute-resolution obligations. OAuth credentials are removed after the corresponding disconnection completes; showcase data is fetched in real time. Ordinary TikTok FILE_UPLOAD sends files directly from the browser to TikTok. When persistent previews are enabled, a separate private source-video copy and cover are retained with the publishing task; deleting that task or all ordinary TikTok data removes application access and queues physical cleanup with retries. Asset-library and remote-source media follow their own storage and deletion controls and are not automatically deleted on publication. Valid deletion requests are handled under applicable law, subject to limited legally required retention." : "我们仅在提供服务及履行安全、防欺诈、法律、会计和争议处理义务所必需的期限内保留个人信息。OAuth 凭据在对应断开流程完成后删除；橱窗数据实时获取。普通 TikTok FILE_UPLOAD 由浏览器直接传输至 TikTok。启用持久预览时，另行保存私有原视频副本和封面并随发布任务保留；删除该任务或全部普通 TikTok 数据后，应用访问立即失效，副本进入带失败重试的物理清理队列。素材库及远程来源媒体遵循各自的存储与删除控制，不会因发布成功而自动删除。有效删除请求依法处理，但法律要求有限保留的情形除外。"}</p>
                  <p className="leading-relaxed mt-3"><strong>{lang === "en" ? "5.3 Data Deletion:" : "5.3 数据删除："}</strong> {lang === "en" ? "You may request deletion of your personal data at any time by contacting us at developer@toryxai.com. We will verify and process valid requests in accordance with applicable law. Disconnecting a TikTok authorization removes or disables the applicable OAuth credential and marks that local binding as disconnected; it does not by itself erase retained account identity or history. After both comment and publishing authorizations are disconnected, the Delete all ordinary TikTok data control or a valid deletion request removes the remaining ordinary-account bindings, publishing tasks, cached comments and replies, synchronization records, and action logs, except where limited continued retention is required for security, legal compliance, or dispute resolution." : "您可以随时通过联系 developer@toryxai.com 请求删除您的个人数据。我们将依照适用法律核验并处理有效请求。断开 TikTok 授权会删除或停用相应 OAuth 凭据，并将该本地绑定标记为已断开；该操作本身不会清除保留的账号身份或历史。评论授权和发布授权均断开后，可通过“删除全部普通 TikTok 数据”或有效的数据删除请求，清除剩余的普通账号绑定、发布任务、评论与回复缓存、同步记录及操作日志，但出于安全、法律合规或争议处理需要继续有限保留的情形除外。"}</p>
                </div>
              </div>
            </ReflectiveCard>
            ...
            <ReflectiveCard className="!rounded-xl">
              <div className="flex items-start gap-4 p-6">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-white/[0.04]"><TikTokLogo className="h-7 w-7 text-white" /></div>
                <div>
                  ...
                  <ul className="list-disc pl-6 space-y-2 mt-1 text-gray-300">
                    <li><strong>video.upload</strong> — {lang === "en" ? "Permission to upload videos and photos for the publishing workflow you initiate" : "在您主动发起的发布工作流中上传视频和图片的权限"}</li>
                    ...
                    <li><strong>user.info.stats</strong> — {lang === "en" ? "Permission to display account-level follower, following, likes, and video statistics" : "展示账号粉丝数、关注数、获赞数及视频数量统计的权限"}</li>
                    <li><strong>video.list</strong> — {lang === "en" ? "Read-only permission to list videos belonging to your authorized TikTok account and display their metadata and statistics" : "以只读方式列出您已授权 TikTok 账号的视频，并展示视频元数据和统计信息的权限"}</li>
                  </ul>
                  <p className="leading-relaxed mt-3 text-sm text-gray-400">{lang === "en" ? 'TikTok API for Business / Accounts API (separate comment authorization):' : 'TikTok API for Business / Accounts API（独立评论授权）：'}</p>
                  <ul className="list-disc pl-6 space-y-2 mt-1 text-gray-300">
                    <li><strong>comment.list</strong> — {lang === "en" ? "Read comments and reply threads on eligible videos for the TikTok account you separately authorize" : "读取您另行授权的 TikTok 账号之符合条件视频上的评论及回复线程"}</li>
                    <li><strong>comment.list.manage</strong> — {lang === "en" ? "Create a reply only when you explicitly enter the reply and submit it" : "仅在您明确输入回复内容并主动提交时创建回复"}</li>
                  </ul>
                  <p className="leading-relaxed mt-3">{lang === "en" ? 'The Business comment authorization is separate from Login Kit and Content Posting authorization. Granting, refreshing, or disconnecting one authorization does not by itself grant, refresh, or disconnect the other.' : 'Business 评论授权与 Login Kit 及内容发布授权相互独立。授予、刷新或断开其中一种授权，不会自动授予、刷新或断开另一种授权。'}</p>
                  ...
                  <ul className="list-disc pl-6 space-y-2 mt-3 text-gray-300">
                    ...
                    <li><strong>{lang === "en" ? "Authorized Account Videos:" : "授权账号视频："}</strong> {lang === "en" ? "Video ID, title or description, cover image, publish time, duration, and view, like, comment, and share counts for videos belonging to your authorized account" : "您已授权账号所拥有视频的视频 ID、标题或描述、封面、发布时间、时长，以及播放、点赞、评论和分享次数"}</li>
                    <li><strong>{lang === "en" ? "Business Comments and Replies:" : "Business 评论与回复："}</strong> {lang === "en" ? "For videos linked to your authorized account, comment and reply identifiers, text, author name or identifier, avatar where provided, status, timestamps, like and reply counts, and the replies you explicitly submit" : "对于与您已授权账号关联的视频，我们可能处理评论及回复标识、正文、作者名称或标识、平台提供的头像、状态、时间、点赞及回复数量，以及您明确提交的回复"}</li>
                  ...
                  <ul className="list-disc pl-6 space-y-2 mt-3 text-gray-300">
                    ...
                    <li>{lang === "en" ? "Authorized-account video metadata and statistics are displayed in a read-only video list and are not used to edit or delete the listed public videos" : "授权账号的视频元数据和统计信息仅用于只读视频列表展示，不用于编辑或删除该列表中的公开视频"}</li>
                    <li>{lang === "en" ? "Business comment data is used to display comment threads and necessary engagement indicators to the authorized user. Star Gaze does not automatically or bulk-send TikTok replies; every outbound reply requires an explicit user action." : "Business 评论数据用于向已授权用户展示评论线程及必要互动指标。Star Gaze 不会自动或批量发送 TikTok 回复；每一条对外回复均须由用户明确触发。"}</li>
                    <li>{lang === "en" ? "Comment data and credentials are isolated by Star Gaze user and connected account. OAuth credentials are available only to restricted server-side processes and are not returned to the browser." : "评论数据及凭据按 Star Gaze 用户和已连接账号隔离。OAuth 凭据仅供受限服务端流程使用，不会返回浏览器。"}</li>
                    ...
                    <li>{lang === "en" ? "For ordinary FILE_UPLOAD, the browser sends the selected local file directly to TikTok. When persistent task previews are enabled, Star Gaze also stores a private source-video copy and cover associated with your task so you can preview it after refreshing. This is not another TikTok publication. Asset-library and remote-source media follow their applicable storage controls." : "对于普通 FILE_UPLOAD，浏览器会将所选本地文件直接发送至 TikTok。启用持久任务预览时，Star Gaze 还会保存与任务关联的私有原视频副本和封面，以便刷新后预览；这不会再次发布到 TikTok。素材库及远程来源媒体遵循各自的存储控制。"}</li>
                  </ul>
                  <p className="leading-relaxed mt-3 font-semibold">{lang === "en" ? 'We do not sell TikTok video or comment data, use it for unrelated advertising, or use comment content to train general-purpose AI models. We disclose it only to service providers acting on our instructions where necessary to operate and secure Star Gaze, or where required by law.' : '我们不会出售 TikTok 视频或评论数据，不会将其用于无关广告，也不会使用评论内容训练通用人工智能模型。仅在运营和保护 Star Gaze 所必需时向受我们指示的服务提供商披露，或依法披露。'}</p>

                  <h3 className="text-lg font-medium text-white mt-4 mb-2">{lang === "en" ? "8.4 Data Retention for TikTok Data" : "8.4 TikTok 数据保留"}</h3>
                  <ul className="list-disc pl-6 space-y-2 mt-3 text-gray-300">
                    <li>{lang === "en" ? "OAuth credentials: stored while the applicable TikTok account remains connected and removed when that account connection is disconnected; expired or revoked credentials may require re-authorization" : "OAuth 凭据：在相应 TikTok 账号保持连接期间存储，并在该账号连接断开时删除；凭据过期或授权撤回后可能需要重新授权"}</li>
                    <li>{lang === "en" ? "Authorized-account video list data: fetched from TikTok on demand and not permanently stored as a separate video library; statistics associated with content published through Star Gaze may remain with the applicable publishing task record until that task is deleted or a valid deletion request is completed" : "授权账号视频列表数据：按需从 TikTok 获取，不作为独立视频库永久存储；通过 Star Gaze 发布内容的统计信息可能随对应发布任务记录保留，直至该任务被删除或有效的数据删除请求处理完成"}</li>
                    <li>{lang === "en" ? "Business comment data: synchronized comment and reply records remain available to the connected Star Gaze account for comment management and audit purposes. After disconnecting both the comment and publishing authorizations, you can use Delete all ordinary TikTok data in TikTok Account Management to transactionally remove the local account bindings, publishing tasks, cached comments, replies, synchronization records, and action logs, or submit a valid deletion request. Limited records may be retained only where necessary for security, legal compliance, or dispute resolution." : "Business 评论数据：已同步的评论及回复记录会在已连接的 Star Gaze 账号中用于评论管理及审计。先完成评论授权和发布授权的撤销后，您可在 TikTok 账号管理中使用“删除全部普通 TikTok 数据”，以事务方式删除本地账号绑定、发布任务、评论与回复缓存、同步记录及操作日志，也可提交有效的数据删除请求。仅在安全、法律合规或争议处理确有必要时有限保留相关记录。"}</li>
                    <li>{lang === "en" ? "Independent comment disconnection: using Disconnect Comments immediately blocks further local use of the Business comment credential and requests revocation from TikTok without deleting the separate publishing account connection or publishing-task history. A protected retry state may be retained when the remote revocation result is uncertain." : "独立断开评论授权：使用“断开评论”会立即阻止本地继续使用 Business 评论凭据，并向 TikTok 请求撤销该凭据，但不会删除独立的发布账号连接或发布任务历史。当远端撤销结果不确定时，系统可能保留受保护的重试状态。"}</li>
                    ...
                    <li>{lang === "en" ? "Video previews: saved private copies remain while the corresponding task exists. Deleting a task or deleting all ordinary TikTok data immediately removes application access and queues its copies for physical cleanup. Failed cleanup is retried; unfinished uploads are also cleaned up. Disconnecting authorization alone does not remove retained publishing-task history or its previews." : "视频预览：已保存的私有副本随对应任务保留。删除任务或删除全部普通 TikTok 数据后，应用访问立即失效，副本进入物理清理队列；清理失败会重试，未完成的上传也会清理。仅断开授权不会删除保留的发布任务历史及其预览。"}</li>
                    <li>{lang === "en" ? "Authorization revocation and deletion: revoking access in TikTok stops future authorized access once detected. In Star Gaze, disconnect the independent comment authorization first and then the publishing authorization so both remote credentials are revoked. After both revocations complete, use Delete all ordinary TikTok data to remove the remaining local ordinary-account records. This local deletion does not delete videos, comments, or replies hosted by TikTok and does not delete separate TikTok Shop data." : "授权撤回与数据删除：在 TikTok 中撤回授权后，系统检测到撤回状态即停止后续授权访问。请先在 Star Gaze 中断开独立的评论授权，再断开发布授权，以撤销两类远端凭据；两项撤权完成后，使用“删除全部普通 TikTok 数据”清除剩余的普通账号本地记录。本地删除不会删除 TikTok 平台托管的视频、评论或回复，也不会删除独立的 TikTok Shop 数据。"}</li>
                  </ul>
                </div>
              </div>
            </ReflectiveCard>
            ...
            <ReflectiveCard className="!rounded-xl">
              <div className="flex items-start gap-4 p-6">
                ...
                <div>
                  ...
                  <p className="leading-relaxed mt-3">{lang === "en" ? `You may disconnect one Page or use “Delete all Facebook data” in Facebook Account Management, revoke access in Facebook Apps and Websites settings, follow the confirmation link returned by Meta’s data-deletion callback, or contact ${APP_BRAND.contactEmail}. Deleting data stored by Star Gaze does not delete videos, posts, comments, replies, or other content hosted by Facebook; Facebook-hosted content must be managed through Facebook or another authorized client that supports the relevant deletion action.` : `您可以在 Facebook 账号管理中解绑单个 Page 或使用“删除全部 Facebook 数据”，通过 Facebook“应用和网站”设置撤销访问，使用 Meta 数据删除回调返回的确认链接，或联系 ${APP_BRAND.contactEmail}。删除 Star Gaze 保存的数据不会删除 Facebook 平台保存的视频、帖子、评论、回复或其他内容；Facebook 平台内容必须通过 Facebook 或其他支持相应删除操作的授权客户端进行管理。`}</p>
