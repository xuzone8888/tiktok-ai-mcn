            ...
            <ReflectiveCard className="!rounded-xl">
              <div className="flex items-start gap-4 p-6">
                ...
                <div>
                  ...
                  <p className="leading-relaxed mt-3"><strong>{lang === "en" ? "5.2 Storage Period:" : "5.2 存储期限："}</strong> {lang === "en" ? "We retain personal information only for as long as necessary to provide the service or satisfy security, fraud-prevention, legal, accounting, and dispute-resolution obligations. OAuth credentials are retained while the applicable account connection remains active and are removed after the corresponding disconnection completes; product showcase data is fetched in real time and is not maintained as a permanent product library. Local files selected for ordinary TikTok FILE_UPLOAD are transferred by the browser directly to TikTok and are not stored by Star Gaze. Media already saved in the Star Gaze asset library or supplied by remote URL follows its own storage and deletion controls and is not automatically deleted merely because a TikTok publication succeeds. Account cancellation and deletion requests are handled in accordance with applicable law, subject to records we must retain for the limited purposes described above." : "我们仅在提供服务或履行安全、防欺诈、法律、会计及争议处理义务所必需的期限内保留个人信息。OAuth 凭据在相应账号连接有效期间保留，并在对应断开流程完成后删除；橱窗商品数据通过 API 实时获取，不作为永久商品库保存。普通 TikTok FILE_UPLOAD 所选择的本地文件由浏览器直接传输至 TikTok，Star Gaze 不会存储该文件；已经保存在 Star Gaze 素材库或通过远程 URL 提供的媒体遵循各自的存储与删除控制，不会仅因 TikTok 发布成功而自动删除。账户注销和数据删除请求将依照适用法律处理，但为上述有限目的必须保留的记录除外。"}</p>
              ...
              <div className="flex items-start gap-4 p-6">
                ...
                <div>
                  ...
                  <ul className="list-disc pl-6 space-y-2 mt-3 text-gray-300">
                    ...
                    <li>{lang === "en" ? "For ordinary FILE_UPLOAD, the browser sends the selected local file directly to TikTok and Star Gaze does not store it. Media selected from the Star Gaze asset library or supplied by remote URL follows the applicable asset-storage and deletion controls." : "对于普通 FILE_UPLOAD，浏览器会将所选本地文件直接发送至 TikTok，Star Gaze 不会存储该文件；从 Star Gaze 素材库选择或通过远程 URL 提供的媒体遵循相应的素材存储与删除控制。"}</li>
                  </ul>
                  ...
                  <ul className="list-disc pl-6 space-y-2 mt-3 text-gray-300">
                    ...
                    <li>{lang === "en" ? "Video files: ordinary local FILE_UPLOAD files are not stored by Star Gaze; asset-library and remote-source media remains subject to the applicable product retention and deletion controls" : "视频文件：普通本地 FILE_UPLOAD 文件不会由 Star Gaze 存储；素材库及远程来源媒体继续适用相应产品的保留与删除控制"}</li>
