      ...
      <main className="relative z-10 py-16">
        <div className="container max-w-4xl mx-auto px-6">
          <div className="mb-12">
            ...
            <p className="text-gray-400">{lang === "en" ? "Updated Date: August 8, 2026" : "更新日期：2026年8月8日"}</p>
          ...
          <div className="space-y-6 text-gray-300">
            ...
            <ReflectiveCard className="!rounded-xl">
              <div className="flex items-start gap-4 p-6">
                ...
                <div>
                  ...
                  <p className="leading-relaxed mt-3">{lang === "en" ? '3.4 TikTok Authorization Revocation and Disconnection: You may revoke Star Gaze\'s access through your TikTok account settings at any time. To remove the connection and stored OAuth credentials from Star Gaze, use the Disconnect action in TikTok Account Management. Publishing-task history and aggregate statistics are separate from the account connection and remain until you delete the applicable task or submit a valid deletion request. Authorization transaction records and other records that must be retained for security, fraud prevention, legal compliance, or dispute resolution may be retained only for the period necessary for those purposes.' : '3.4 TikTok 授权撤回与解绑：您可以随时通过 TikTok 账户设置撤回 Star Gaze 的访问授权。如需从 Star Gaze 中移除账号连接及已存储的 OAuth 凭据，请在 TikTok 账号管理中使用“断开连接”操作。发布任务历史及聚合统计与账号连接相互独立，会保留至您删除相应任务或提交有效的数据删除请求。出于安全、防欺诈、法律合规或争议处理所必需的授权事务记录及其他记录，仅在实现上述目的所需的期限内保留。'}</p>
                </div>
              </div>
            </ReflectiveCard>
            ...
            <ReflectiveCard className="!rounded-xl">
              <div className="flex items-start gap-4 p-6">
                ...
                <div>
                  ...
                  <p className="leading-relaxed mt-3"><strong>{lang === "en" ? "5.2 Storage Period:" : "5.2 存储期限："}</strong> {lang === "en" ? "We retain personal information only for as long as necessary to provide the service or satisfy security, fraud-prevention, legal, accounting, and dispute-resolution obligations. OAuth credentials are retained while the applicable account connection remains active and are removed when that connection is disconnected; product showcase data is fetched in real time and is not maintained as a permanent product library; video files are deleted from our servers after successful upload to TikTok. Account cancellation and deletion requests are handled in accordance with applicable law, subject to records we must retain for the limited purposes described above." : "我们仅在提供服务或履行安全、防欺诈、法律、会计及争议处理义务所必需的期限内保留个人信息。OAuth 凭据在相应账号连接有效期间保留，并在该连接断开时删除；橱窗商品数据通过 API 实时获取，不作为永久商品库保存；视频文件在成功上传至 TikTok 后从我们的服务器删除。账户注销和数据删除请求将依照适用法律处理，但为上述有限目的必须保留的记录除外。"}</p>
                  <p className="leading-relaxed mt-3"><strong>{lang === "en" ? "5.3 Data Deletion:" : "5.3 数据删除："}</strong> {lang === "en" ? "You may request deletion of your personal data at any time by contacting us at developer@toryxai.com. We will verify and process valid requests in accordance with applicable law. Disconnecting a TikTok account removes that account connection and its stored OAuth credentials; publishing-task history, aggregate statistics, authorization transaction records, and other retained records are deleted through their applicable product controls or a valid deletion request, except where continued retention is required for security, legal compliance, or dispute resolution." : "您可以随时通过联系 developer@toryxai.com 请求删除您的个人数据。我们将依照适用法律核验并处理有效请求。断开 TikTok 账号连接会删除该账号连接及其已存储的 OAuth 凭据；发布任务历史、聚合统计、授权事务记录及其他保留记录，可通过相应产品删除入口或有效的数据删除请求删除，但出于安全、法律合规或争议处理需要继续保留的情形除外。"}</p>
                </div>
              </div>
            </ReflectiveCard>
            ...
            <ReflectiveCard className="!rounded-xl">
              <div className="flex items-start gap-4 p-6">
                <div className="p-3 rounded-lg bg-cyan-500/10 text-white shrink-0"><TikTokLogo className="h-6 w-6" /></div>
                <div>
                  ...
                  <ul className="list-disc pl-6 space-y-2 mt-3 text-gray-300">
                    ...
                    <li>{lang === "en" ? "Business comment data: synchronized comment and reply records remain available to the connected Star Gaze account for comment management and audit purposes until removed through applicable product controls or a valid deletion request, subject to limited security, legal-compliance, and dispute-resolution retention" : "Business 评论数据：已同步的评论及回复记录会在已连接的 Star Gaze 账号中用于评论管理及审计，直至通过相应产品删除入口移除或有效删除请求处理完成；安全、法律合规及争议处理所需的有限保留除外"}</li>
                    ...
                    <li>{lang === "en" ? "Authorization revocation and deletion: revoking access in TikTok stops future authorized access once detected; use Disconnect in Star Gaze to remove the stored account connection and OAuth credentials. Other retained TikTok-related records follow the deletion controls and limited retention purposes described in Sections 3.4 and 5." : "授权撤回与数据删除：在 TikTok 中撤回授权后，系统检测到撤回状态即停止后续授权访问；请在 Star Gaze 中使用“断开连接”删除已存储的账号连接及 OAuth 凭据。其他保留的 TikTok 相关记录遵循第 3.4 条和第 5 条所述的删除入口及有限保留目的。"}</li>
                  </ul>
                </div>
              </div>
            </ReflectiveCard>
