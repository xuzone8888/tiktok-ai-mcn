...
import { ArrowLeft, Sparkles, Shield, Globe, FileText, Users, UserCheck, AlertTriangle, BookOpen, Scale, Gavel, RefreshCw, Phone, CreditCard, Megaphone, Receipt, ShieldX, Layers, Baby, MoreHorizontal, DollarSign } from "lucide-react";
      ...
      <main className="relative z-10 py-16">
        <div className="container max-w-4xl mx-auto px-6">
          <div className="mb-12">
            ...
            <p className="text-gray-400">{lang === "en" ? "Last Updated: September 11, 2026" : "最后更新日期：2026年9月11日"}</p>
            ...
            <ReflectiveCard className="!rounded-xl">
              <div className="flex items-start gap-4 p-6">
                ...
                <div>
                  ...
                  <p className="leading-loose">{lang === "en" ? '16.1 Service Description: Star Gaze integrates with the TikTok platform through three feature categories: (a) Login Kit and the video.list scope — for connecting your own TikTok account and displaying its videos and statistics in a read-only list; (b) Content Posting API — for directly publishing AI-generated videos and photos to your TikTok account (Direct Post mode); and (c) TikTok Shop API — for publishing shoppable videos with product links from your product showcase. You must authorize the corresponding TikTok account before using each service.' : '16.1 服务描述：Star Gaze 通过三类功能与 TikTok 平台集成：(a) Login Kit 及 video.list 权限 — 连接您自己的 TikTok 账号，并以只读列表展示该账号的视频和统计信息；(b) 内容发布 API — 将 AI 生成的视频和图片直接发布到您的 TikTok 账户（Direct Post 模式）；(c) TikTok Shop API — 发布带有橱窗商品链接的带货视频。您需要先授权相应的 TikTok 账户才能使用各项服务。'}</p>
                  ...
                  <p className="leading-loose mt-3">{lang === "en" ? '16.4 Read-Only Video List: When you grant video.list permission, Star Gaze may display videos belonging to your authorized TikTok account, including cover, publish time, duration, and view, like, comment, and share counts. This new list does not edit or delete the public videos it displays. Existing publishing-task management remains part of the separately authorized Content Posting workflow.' : '16.4 只读视频列表：当您授予 video.list 权限后，Star Gaze 可展示您已授权 TikTok 账号所拥有的视频，包括封面、发布时间、时长，以及播放、点赞、评论和分享次数。该新增列表不会编辑或删除所展示的公开视频。既有发布任务管理仍属于另行授权的内容发布工作流。'}</p>
                  <p className="leading-loose mt-3">{lang === "en" ? '16.4A Independent Business Comment Authorization: Comment management uses a separate TikTok API for Business account-holder authorization for comment.list and, when you choose to reply, comment.list.manage. This authorization is separate from Login Kit, Content Posting, Display API, and TikTok Shop authorization.' : '16.4A 独立 Business 评论授权：评论管理使用独立的 TikTok API for Business 账号持有人授权，读取评论使用 comment.list，您选择回复时使用 comment.list.manage。该授权与 Login Kit、内容发布、Display API 及 TikTok Shop 授权相互独立。'}</p>
                  <p className="leading-loose mt-3">{lang === "en" ? '16.4B User-Initiated Replies: Star Gaze does not automatically or bulk-send replies. You are responsible for each reply you enter and explicitly submit, and for ensuring that it complies with applicable law and TikTok rules.' : '16.4B 用户主动回复：Star Gaze 不会自动或批量发送回复。您应对自己输入并明确提交的每条回复负责，并确保其符合适用法律及 TikTok 规则。'}</p>
                  <p className="leading-loose mt-3">{lang === "en" ? '16.4C Separate Disconnection: Disconnect Comments revokes or disables only the Business comment authorization. It does not disconnect the separate publishing authorization or delete publishing tasks. If TikTok revocation cannot be confirmed immediately, Star Gaze blocks local use while retaining a protected retry and audit record.' : '16.4C 独立断开：使用“断开评论”仅撤销或停用 Business 评论授权，不会断开独立的发布授权或删除发布任务。如无法立即确认 TikTok 远端撤销结果，Star Gaze 会阻止本地继续使用该凭据，并保留受保护的重试及审计记录。'}</p>
                  <p className="leading-loose mt-3">{lang === "en" ? '16.5 Showcase Product Data (TikTok Shop): Product information displayed on the platform is sourced from your TikTok Shop showcase, including product name, price, commission rate, and images. This information is used solely to help you select products for shoppable video creation and is not stored permanently on our servers.' : '16.5 橱窗商品数据（TikTok Shop）：平台展示的商品信息来源于您的 TikTok Shop 橱窗，包括商品名称、价格、佣金率和图片。此信息仅用于帮助您选择商品进行带货视频创作，不会永久存储在我们的服务器上。'}</p>
                  <p className="leading-loose mt-3">{lang === "en" ? '16.6 Branded/Commercial Content Disclosure: If your published content promotes a brand, product, or service — whether through paid partnerships, gifted collaborations, or any form of compensation — you must enable TikTok\'s "Disclose commercial content" setting. Star Gaze may apply this setting automatically when publishing shoppable videos with product links.' : '16.6 品牌/商业内容披露：如果您发布的内容推广品牌、产品或服务 — 无论是通过付费合作、赠品合作还是任何形式的报酬 — 您必须启用 TikTok 的"披露商业内容"设置。当发布带商品链接的带货视频时，Star Gaze 可能会自动应用此设置。'}</p>
                  <p className="leading-loose mt-3">{lang === "en" ? '16.7 Data and Authorization Revocation: You may revoke Star Gaze\'s access through your TikTok account settings at any time. In Star Gaze, comment and publishing authorizations are disconnected independently so each remote credential is revoked safely. After both revocations complete, you may use “Delete all ordinary TikTok data” in TikTok Account Management to transactionally delete local ordinary-account bindings, publishing tasks, cached comments and replies, synchronization records, and action logs. This does not delete content hosted by TikTok or separate TikTok Shop data. Limited records may be retained only as described in our Privacy Policy.' : '16.7 数据和授权撤回：您可以随时通过 TikTok 账户设置撤回 Star Gaze 的访问授权。在 Star Gaze 中，评论授权和发布授权需分别断开，以安全撤销各自的远端凭据。两项撤权完成后，您可在 TikTok 账号管理中使用“删除全部普通 TikTok 数据”，以事务方式删除本地普通账号绑定、发布任务、评论与回复缓存、同步记录及操作日志。该操作不会删除 TikTok 平台托管的内容，也不会删除独立的 TikTok Shop 数据；有限记录仅可按照隐私政策所述目的保留。'}</p>
                  <p className="leading-loose mt-3">{lang === "en" ? "16.8 Related Terms:" : "16.8 相关条款："}</p>
            ...
            <ReflectiveCard className="!rounded-xl">
              <div className="flex items-start gap-4 p-6">
                ...
                <div>
                  ...
                  <p className="leading-loose">{lang === "en" ? '19.1 Primary persistent storage for account, authorization, publishing, comment, and statistics data is hosted in the United States. Star Gaze\'s public application and API services are hosted in China and may transiently process personal information and platform-related requests and data as necessary to operate, support, diagnose, and secure the services. Relevant data may also be transmitted to TikTok and the service providers disclosed in the Privacy Policy.' : '19.1 账号、授权、发布、评论及统计数据的主要持久化存储位于美国。Star Gaze 的公开应用和 API 服务托管于中国，可能在运营、支持、诊断和保护本服务所必需的范围内，临时处理个人信息以及平台相关请求和数据。相关数据还可能按功能需要传输至 TikTok 及隐私政策披露的服务提供商。'}</p>
