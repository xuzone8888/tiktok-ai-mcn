...
export async function POST(request: NextRequest) {
  ...
  if (!isLocalOrigin(origin) && process.env.FACEBOOK_LOCAL_UPLOAD_FALLBACK !== 'true') {
    return NextResponse.json(
      { success: false, error: 'Facebook local upload fallback is only available on localhost.' },
      { status: 403 }
    )
  }
  ...
  const url = new URL(`/api/facebook/upload/local-video/${encodeURIComponent(id)}`, origin)
