import { NextRequest, NextResponse } from 'next/server'

import { createClient } from '@/lib/supabase/server'
import {
  getEnabledSocialCommentPlatforms,
  isSocialCommentsApiEnabled,
} from '@/lib/social-comments/feature-flag'
import { getApiMessage, getRequestLang } from '@/lib/social-comments/i18n'
import { getSocialCommentAccounts } from '@/lib/social-comments/service'
import { isSocialPlatform } from '@/lib/social-comments/types'

export const dynamic = 'force-dynamic'

export async function GET(request: NextRequest) {
  if (!isSocialCommentsApiEnabled()) {
    return NextResponse.json({ error: 'Not found', code: 'not_found' }, { status: 404 })
  }

  const lang = getRequestLang(request.headers)

  try {
    const supabase = await createClient()
    const { data: { user }, error } = await supabase.auth.getUser()
    if (error || !user) {
      return NextResponse.json({ error: getApiMessage('unauthorized', 'Unauthorized', lang), code: 'unauthorized' }, { status: 401 })
    }

    const enabledPlatforms = getEnabledSocialCommentPlatforms()
    const requestedPlatform = request.nextUrl.searchParams.get('platform')
    if (requestedPlatform && (!isSocialPlatform(requestedPlatform) || !enabledPlatforms.includes(requestedPlatform))) {
      return NextResponse.json({ error: 'Not found', code: 'not_found' }, { status: 404 })
    }

    const accounts = await getSocialCommentAccounts(
      user.id,
      requestedPlatform && isSocialPlatform(requestedPlatform) ? [requestedPlatform] : enabledPlatforms
    )
    return NextResponse.json({ accounts })
  } catch (error) {
    console.error('Social comment accounts API error:', {
      name: error instanceof Error ? error.name : 'Error',
      code: 'internal_error',
    })
    return NextResponse.json(
      { error: getApiMessage('internal_error', 'Server error', lang), code: 'internal_error' },
      { status: 500 }
    )
  }
}
