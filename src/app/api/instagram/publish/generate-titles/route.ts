...
const FALLBACK_HASHTAGS = ['#Instagram', '#Reels', '#视频分享']
...
function trimTitle(value: string) {
  return value.length > INSTAGRAM_TITLE_MAX_LENGTH ? value.slice(0, INSTAGRAM_TITLE_MAX_LENGTH) : value
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (result.success && result.titles?.length) {
      return NextResponse.json({
        ...
        titles: result.titles.map((item) => ({
          ...item,
          title: trimTitle(item.title || item.combined || ''),
          combined: trimTitle(item.title || item.combined || ''),
        })),
        provider: 'doubao',
      })
    }
    ...
    console.error('Instagram generate titles error:', error)
    return NextResponse.json({
      success: true,
      fallback: true,
