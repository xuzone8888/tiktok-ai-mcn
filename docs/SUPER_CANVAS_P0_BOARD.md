...
🔴 **未做的一项**:**生产 UI 复验**。本批全部证据都是离线闸门 + 代码复核,
**没有在真实浏览器里拖过一次选区**。本轮浏览器自检返回 `NO-FRAME` + 匿名态(见批 0 方法学说明),
需用户把窗口置前并登录后补。零成本可验(裁剪不扣费),但**必须验**——
选区交互、CORS 是否真的没污染 canvas、上传后节点是否真出现,这三件离线闸门都证明不了。
