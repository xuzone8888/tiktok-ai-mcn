...
## 🔴 两个影响生产的既有缺陷(本轮查实;**2026-08-10 已裁决并落代码,迁移待生产执行**)

> **用户裁决(2026-08-10)**:缺陷甲走**方案 B**(放宽保存闸白名单);缺陷乙一并修;
> 「推为参考」那处生产缺陷的修复**攒着与批 5/批 6 一起发版**,不单独发。
>
> **落地**:`5f0429a` —— 新增迁移 `20260810_canvas_media_readiness_blueprint_assets.sql`
> (第四条放行分支 + 只读 helper `canvas_blueprint_media_key_ref_v1`)与
> `image-generation-worker.ts` 两处查询加 `.or(source.is.null,source.neq.canvas)`。
>
> 🔴 **两条未完成事项**:
> 1. **迁移必须由用户在 Supabase dashboard 执行**(生产无 `exec_sql` RPC)。
>    ⚠️ 本地 dev 与生产**同一个 Supabase**,所以 AI 无法「先在本地库试一次」——
>    试就是改生产。**执行前该修复无法做 UI 复验**;执行后请回来跑一遍
>    「历史面板加一张蓝图素材 ⇒ 应保持『已保存』」。
> 2. **本地无直连 Postgres 也无 psql,未能做真正的 Postgres 语法解析。**
>    已做的替代校验:①主函数体与原版逐行 `diff`,除预期 8 行外一字未改;
>    ②按迁移里同一套字段集合与匹配条件在真实蓝图行上复刻验证(肇事 key 放行、
>    蓝图暴露的 6 个 key 全放行、伪造同前缀 key 与他人 uid 的 key 仍拒、
>    `xproducts/` 不误配 `products/`)。语法问题会在 dashboard 执行时立即暴露且无副作用。
