...
- **R2-Q3 · 🔴 P1 功能缺失:「删除 running 节点三选一」根本没实现**(2026-08-08 实测,待裁决)
  ...
  - **✅ 已实现(2026-08-08,本轮)**:
    ...
    🆕 **2026-08-10 更正 verifier 计数**:现在是 **32 个脚本**,已知红 **2 个**(同上两条环境依赖)。
    ⚠️ 期间 `generation-frontend` 曾红过 —— 裁剪节点按修复不再写 `data.title`,而断言还要求它,
    已**改断言不改代码**并加了负控(把 title 加回去立刻报红)。
    **别照「29 绿」这个数字对照**:红的曾是 3 个而绿数巧合仍是 29,照数字看会误判成「还是那两个已知失败」。
...
- **P1-Q5 · ✅ 已裁决(2026-08-09,用户)· #67 商品节点走方案 A:完整做,含视觉卖点解析**

  用户选 A(而非「先做上传图+手填卖点」)。⇒ **批 5 的范围 = 上传图 → 调 `/api/studio/analyze-product`
  出卖点卡 → 存进节点**,真正打通电商轨入口。

  **前置件批 0 已实证全部现成,且不碰资金链路**:
  - 持久化槽位**已在 schema 里**,`CanvasNodeDataSchema.media`(`ossKey`/`posterKey`)对所有节点类型开放,
    `refs.assetId` + `refs.assetTable`(`ai_models`|`canvas_assets`)成对约束就位 —— **无需 schema 迁移**;
  ...
  - 出口 `/api/studio/analyze-product`(51 行)**已登录鉴权(无登录 401)、零积分扣费、不写 `generations`**。

  ⚠️ **两点必须在批 5 开工时守住**:
  1. 该路由**对用户零扣费,但对我方有厂商成本**(豆包视觉 70s 预算 / Qwen 35s 降级,`maxDuration=120`)。
     必须防重复触发(同一组图不要每次渲染都重解析),并给「重新解析」一个显式入口而不是自动跑。
  2. 这是画布**第一次调 `/api/canvas/*` 之外的路由**。它与 #81/#85 性质完全不同 ——
     analyze-product **不是计费路由、不写 generations、不进对账车道**,所以不触铁律 1 的「零 fork」。
     但仍应在批 5 的 verifier 里断言:画布对**计费类**路由的引用数保持为 0。

- **P1-Q4 · ✅ 已裁决并已落笔(2026-08-09,用户)· 采纳方案 A:#92 整项随 #81 改判「延 P2」**

  CHECKLIST 第 92 行已改 `延/P2` 并写入三条理由(高清半无所指 / 图生图半纯发现性零新能力 /
  承载面在低 zoom 下也会渲染可点按钮);「真正值得补的发现性提示应落在引用区 0 张时的空位」
  这条**留 P2 复议**,不在本期偷偷改规格。统计表 + CLAUDE.md 同步,`reconcile` 绿:
  `220(做 163/裁 31/延 26)`,**P1 由 49 降为 48**。
  ⇒ **P1 剩余缺口只剩 3 项:#182(批 4)、#67(批 5)、#211(批 6)。**

- **P1-Q4(原提请全文,保留备查)· #92 图片空态快捷:在 #81 延 P2 之后它还剩什么**

  CHECKLIST #92 规格逐字是「图片空态快捷:**图生图 / 图片高清**」(做/P1,节点内空态)。
  批 3 实证后有两点:

  1. 🔴 **「图片高清」那一半已经无所指** —— #81 已按 P1-Q3 改判延 P2,该快捷没有可指向的功能。
     照规格硬做就是造一个 no-op 按钮。
  2. **「图生图」那一半是纯发现性,零新能力。** 选中空图片节点时生成面板**本来就完整可用**
     (`media-node.tsx` 渲染面板不看 `media`),文生图一步不缺;画布的「图生图」本质就是
     **连一个上游图片节点**。所以快捷位唯一说得通的动作是「一次性拿到一张图并建成本节点的上游」。
     ⚠️ 且**绝不能建空的上游节点** —— 那会把一个可提交的节点变成必报错的节点
     (`canvas-generation-context.tsx` 空图片输入直接抛错)。
     真正缺的是提示:引用区 0 张时整块隐身、lock-hint 只对视频说话,用户不知道「连张图进来就是图生图」。
     但那个提示更该放在**引用区空位**(有图之后依然成立),而不是虚线空态框(有图就消失)——
     那已经等于改规格了。
  3. 另有一处承载面疑虑:空态分支排在低 zoom 分支**之前**,按字面把按钮塞进 `MediaEmpty`,
     zoom<0.4 时也会渲染可点按钮,与「低 zoom 整体降级」取向相冲。

  **这属于铁律 10 的功能取舍,AI 不擅改。三选一**:
  | | 方案 | 说明 |
  |---|---|---|
  | **A(建议)** | **整项随 #81 改判延 P2** | 与 #81/#85 同批同判据。剩下那半是纯发现性,不改变用户能做成的任何事 |
  | B | 只做「图生图」半,并允许把落点从节点空态改到**引用区空位** | 补的是真实的发现性缺口,且位置在有图之后依然成立;但这是**收窄+挪位**,须你明确同意 |
  | C | 维持原规格硬做两个按钮 | 「高清」那个当前无功能可接,会做出 no-op —— **不建议** |

- **P1-Q3 · ✅ 已裁决并已落笔(2026-08-09,用户)· 采纳方案 A:#81/#85 改判「延 P2 · 外部条件不具备」**

  CHECKLIST 第 81/85 行已改 `延/P2` 并写入完整实证理由;**#85 那条错备注已改写**
  (原文「复用 /api/generate/image 现成 nine_grid 分支…纯 UI 露出 S」——比 R2-Q4 判的更错:
  不是工作量估错,是**可复用物不存在**)。两张统计表 + CLAUDE.md 期次表同步,
  `reconcile` 绿:`220(做 164/裁 31/延 25)`,**P1 由 51 降为 49**。
  「高清是否顶成 2k」那问随方案 A 作废(用户答复:不顶,用户选几 k 就几 k)——留档备将来排期改上游时用。
  ⇒ **P1 收尾剩余批次:批 3(#43/#92)、批 4(#182)、批 5(#67)、批 6(#211)。**

- **P1-Q3(原提请全文,保留备查)· #81/#85 的「现成分支」根本不存在,我在 P1-Q1 里判轻了**

  **🔴 先更正 P1-Q1 的一个错判。** 我在 P1-Q1 写过「upscale/nine_grid 两个分支的实质极薄,
  接入无需定价、无需改价、落地范围收敛为扩 mode + 复用模板」。**深挖之后这个结论站不住。**
  当时我只读了 `route.ts` 那几段分支代码就下了「可复用」的判断,**没有验证它在画布链路里够不够得着**。
  九路并行测绘 + 三路对抗审查(1.31M tokens)后逐条实证如下:

  | # | 实证 | 证据 |
  |---|---|---|
  | 1 | **`mode` 从来不进上游请求体。** 上游参数结构体 `VideoPlatformImageParams` 只有 6 个字段,**没有 mode**。所以 mode 不可能"透传"给厂商 | `src/lib/video-platform-image-api.ts:33-42` |
  | 2 | **主路由让 mode 生效的唯一机制是换 prompt** —— 即 `getOpenAIEditPrompt` 那两段固定英文模板 | `src/app/api/generate/image/route.ts:389` |
  | 3 | 🔴 **而那段模板只在「用户提示词为空」时才套用**:函数第 2 行就是 `if (userPrompt) return userPrompt;` | `route.ts:69-71` |
  | 4 | 🔴 **画布提示词恒非空**:intent 强制 `normalizedTextSchema(2, 8_000)`,最短 2 码点 | `src/lib/canvas/generation-intent.ts:204` |
  | 5 | ⇒ **把模板原样搬到画布是 no-op。**「复用现成分支」这件事**在画布上不存在可复用的东西** | 3+4 |

  **要让 #81/#85 真的做出区别,画布必须自己发明一套主路由没有的语义** ——
  「模板 **+** 用户提示词」拼接(主路由是「模板 **或** 用户提示词,绝不并存」)。
  而这一拼,三条路全都撞墙:

  - **客户端拼进 `intent.prompt`** → 服务端会用「本节点 title + 上游文本节点 title」重算
    `authoritativePrompt` 并要求**逐字节全等**,不等即 409 `CANVAS_PROMPT_CHANGED`。
    (`generation-service.ts:409-425`)
  - **把模板作为字段放进 intent** → 改变规范化 JSON → **改指纹** → 对已落盘未决的 actionId
    做恢复重放时 `begin_canvas_generation_v1` 抛 `different fingerprint` → 409 `ACTION_ID_CONFLICT`。
    (`20260718_canvas_p1_generation_api.sql:428-435`)
  - **只能在服务端 `submitImage` 里拼** → 技术上可行,**但会造出一个「花了钱买到的真实输入」既不进 `spec`、
    不进 `generations.prompt`、也不进 `request_fingerprint`** —— 与代码自己写死的不变量直接冲突:
    `generation-service.ts:930` 注释原文是 *"Exact normalized intent is the immutable provider-bound specification."*
    这触的是**铁律 2「generations = 执行状态唯一真相源」**,不是风格问题。

  **另有两件独立的事也要拍板**:

  - **计价语义**:quick-gen 的 upscale **强制把 resolution 顶成 2k**(`quick-gen/page.tsx:1044`)。
    画布若照做,同一个「高清」按钮的扣费就从 **5 分变 10 分**(`IMAGE_RESOLUTION_CREDITS` = 1k:5 / 2k:10 / 4k:15)
    —— **属计价行为变更,触铁律 9**。若不照做,用户在 1k 档点「高清」拿回的还是 1k 图,"高清"名不副实。
  - **共用链路**:把模板抽成共享纯模块要改 `src/app/api/generate/image/route.ts`,
    而那是 **quick-gen / image-factory 共吃**的链路(与 #78 撞的是同一堵墙)。
    不改就得把两段文案复制进画布 lib —— **两份模板必然漂移**。

  **我的建议(三选一,请拍板)**:

  | 方案 | 说明 | 代价 |
  |---|---|---|
  | **A(建议)#81/#85 改判「延 P2 · 外部条件不具备」** | 与 #78 及 2026-08-09 那 8 项**同一判据**:不是工时问题,是上游能力面根本没有 mode 这个概念,画布够不着的不是"分支"而是"能力"。P1 由 51 降为 **49** | 电商轨少两件工具;但**不会**为了凑功能而造出一条不进幂等身份的付费输入 |
  | B 只做 nine_grid,服务端拼模板 | 九宫格对电商更刚需;接受「模板不进指纹」 | 破铁律 2 的不变量;将来对账/复现要多一份口径 |
  | C 全做,并把模板纳入 intent + 升指纹版本 | 语义最干净,模板进 spec、可复现 | **要动 `CANVAS_GENERATION_FINGERPRINT_VERSION`**,而升版会让**所有在途未决 intent 的恢复重放失效** —— 风险远大于收益,**我倾向否决** |

  ⚠️ 无论选哪个,**CHECKLIST #85 备注「复用 /api/generate/image 现成 nine_grid 分支…纯 UI 露出 S」都是错的**,
  且比 R2-Q4 判定的更错:R2-Q4 说「工作量估计错了」,实情是**那个可复用物根本不存在**。该备注须改写。
...
| 分类 | 项 |
|---|---|
| **确实缺** | #43 / #67 / #82 / #92 / #182 / #211(6 行 6 项) |
| **其实有** | **#83 整图重生成**(1 项,**批 0 首轮误判为「确实缺」,批 1 动手时发现并更正**,见下方「一处自我更正」) |
| **判据变了** | **#81+#82+#83 的「必须先建 NodeToolbar」前提不成立**;**#78 比文档写的更严重**;**#182 不需要扩契约**;**#67 的前置件全部现成且零扣费**(详见下) |

> ### 🔴 一处自我更正:#83 其实早就有了
>
> 批 0 首轮把 #83 判成「确实缺」,依据是 `整图`=0、`重生成` 仅命中注释、`regenerate`=0。
> **这个判法本身错了** —— 落到界面上的字是**「生成新版本」**,三个 grep 词一个都不沾。
>
> 实情:`generation-controls.tsx` 的主生成按钮在 `generation?.status === "completed"` 时
> 文案即变为「生成新版本」,而 `actionDisabled`(同文件 `const actionDisabled =`)
> **不含 `completed` 项**,所以产物出来之后按钮照常可点,按当前参数重跑一次整张图 ——
> 这正是 #83 要的能力,且它天然走 `onGenerate("button")`,**#185 同意闸与三道价格闸全部适用**。
> 生产 bundle 里「生成新版本」在 canvas chunk **命中**,即线上就有。
>
> **教训(比这一项本身重要)**:批 0 声称的「特征串全仓 grep」这一重交叉,
> **强度完全取决于选词**;选错词会把「有」读成「没有」,方向与本项目惯有的
> 「说有实无」正好相反。凡结论是「缺」的,必须**再按渲染面读一遍代码**,
> 不能只信 grep 空结果。#43/#67/#92/#182/#211 五项已按此复核过
> (#67/#92 是直接通读组件渲染面、#211 是直接读 `canvas-chrome-policy.ts` 的 `enabled:false`、
> #43 另跑了「已更新/上游/过期/变更/Badge/角标」六个近义词,#182 通读了提示词 textarea),
> 结论不变;**#83 是唯一一处翻案**。
>
> 因此 #83 **不再需要开发**,批 1 只做 #82;并已在 `generation-frontend` verifier 里
> 把「完成后仍可重生成」钉成断言,防止将来有人把 `completed` 加进 `actionDisabled`
> 而悄悄弄丢这项能力,也防止再有人误判后**另开一条绕过同意闸的重生成路径**。
...
| **83** 整图重生成 | ~~确实缺~~ → **其实有** | **已翻案,见上方「一处自我更正」。** 首轮据 `整图`=0 / `重生成` 仅注释 / `regenerate`=0 判缺,但界面用词是**「生成新版本」**:主生成按钮在 `status==="completed"` 时即此文案,且 `actionDisabled` 不含 `completed` 故照常可点;生产 canvas chunk 命中该串 |
...
0. **(结论已被批 1 落地采纳)** 用户裁决:走「复用现成产物动作行」,不新建 `NodeToolbar`。
...
## P1 收尾 · 批 1:#82 裁剪(状态=待审,离线闸门已过,**未做生产 UI 复验**)

**范围只剩 #82** —— #83 经批 1 动手时复核为「其实有」(见批 0 的自我更正),不再开发。

**承载面**:按用户裁决走**现成的产物动作行**(`generation-controls.tsx` 里 下载/全屏/去发布 那条),
不新建 `NodeToolbar`。图片节点该行由 2 键变 3 键(下载/全屏/裁剪),视频侧本来就是 3 键,
⇒ **零高度增量**,不加剧「1352×642 面板溢出停靠位 66px」那条已知摩擦(§负二④)。

**实现要点(与钱、与数据完整性相关的部分)**:
- **纯前端、零扣费**:新组件 `src/components/canvas/nodes/generation-crop-dialog.tsx` 不引用任何
  生成/估价端点(verifier 已断言),像素处理全在浏览器。
- **`crossOrigin="anonymous"` 是硬要求**:产物在 `media.toryxai.com`(OSS)与站点不同源,
  不带它 `<canvas>` 会被污染、`toBlob` 抛 `SecurityError`。**动手前已实测该 bucket 回
  `Access-Control-Allow-Origin: *` / `Allow-Methods: GET, HEAD, OPTIONS`**,方案才成立;
  这条是运行时才炸、离线闸看不见的,故单独进 verifier。
- **按原图像素裁,不按显示像素裁**:展示区被 `max-h/max-w` 压过,选区一律以 0-1 归一化存,
  导出时再乘 `naturalWidth/naturalHeight`,否则会得到一张比原图小得多的图。
- **结果走画布已有的上传链路**(`uploadCanvasFile` → `/api/canvas/uploads/*`),与拖文件进画布同一条路,
  因此配额、10MB 图片上限、object key 归属校验自动生效;导出 JPEG(q=0.92)而非 PNG,
  正是为了不撞那个 10MB 上限。上传前另做一次本地体积预检,超限直接给人话提示而不是让请求失败。
- 🔴 **结果落成独立新节点、刻意不连线**:画布里 A→B 的边语义是「A 是 B 的**生成参考图**」,
  而裁剪产物并非由原图生成;连上去会让用户在新节点上一按生成就把原图当参考送进去 ——
  **那是要花钱才发现的错**。新节点位置在源节点右侧并再下移一格,避免与「推为参考」建的节点精确重叠。
- 上传成功但建节点失败(只读/达上限)时**明说「已裁剪，但新建节点失败」**,不假装无事发生 —— 文件确实已在 OSS 上。

**闸门**:`tsc` 绿、`build` 绿(exit 0)、`generation-frontend` **149→163**(新增 14 条:#82 十一条 + #83 三条)。
其中「裁剪结果不连线」那条做过**负控验证**——把 `addNode` 改成 `addNodeAndEdge` 后 verifier 确实报红,
再还原(否定式断言若写错会永远为真,等于没守,故必须负控过一次)。

**生产 UI 复验**:✅ **由用户于 2026-08-09 自行复检确认通过**。
⚠️ **证据口径要说清**:这一条是**用户口头确认**,不是 AI 观察到的 —— 本窗口的浏览器会话
自检返回 `NO-FRAME` + 匿名态(见批 0 方法学说明),AI 侧**未曾亲自渲染过裁剪弹层**。
故本项在看板上按「用户复检」记账,与 AI 亲验的项(如 #84 全屏预览那批)不是同一等级的证据。
若后续出现与裁剪相关的异常,**不要因为这里写着「已复检」就跳过重验**。

## P1 收尾 · 批 3:#43 dirty 角标(状态=待审,离线闸门已过,**未做生产 UI 复验**);#92 提请裁决

### #43 已落地 —— 并更正了 CHECKLIST 原备注的做法

**原备注写的做法是「节点存上游 generationId 快照,比对即知(新建 S)」,两处都不对**
(仅做法更正,**做/裁/期次一律未动**,已同步 CHECKLIST 第 43 行):

1. **不需要新存快照 —— 它早就在文档里。** 每次提交都会把完整 intent 写进 `params.generation`,
   其 `inputs[]` 就是「这次用的上游」逐项快照(文本存 `textSha256`、图片存 `ossKey`,
   自身跑过生成的另带 `generationId`)。而 `clearPersistedGenerationIntent` **全仓只有两个调用点**
   —— `canvas-generation-context.tsx:1175`(catch 内且仅 `definitive===true`)与 `:1251`(用户点「放弃这次提交」);
   **成功路径不清除**。所以产物产生后它仍在文档里。
   ⇒ #43 做成**纯派生、零文档写入**:不新增 `params` 键、不新增任何文档写。
   **这一点是有代价意识的选择**:任何自动写文档都要过写者租约、会往 undo 栈里插一条用户没做过的步骤
   并清空重做栈、且只读态下会静默失败(角标永远不亮)。
2. **只比 `generationId` 会漏检。** 上传的图、#82 裁剪出来的图、从素材库推过来的图,
   `refs.generationId` 恒为 null(只有节点自己跑过生成才写)。换图恰恰是最常见的上游变更方式之一。
   ⇒ 判据是 **`ossKey` 与 `generationId` 一起比**;另外 `ossKey` 不变而 `generationId` 补上
   (对账把同一份产物重新登记)**不判 dirty**,否则一次无害回写就点亮角标。

**实现要点**:
- 判定是零依赖纯函数 `compareGenerationInputSnapshots`,放在 `generation-input-order.ts`
  ——「上游顺序唯一真相源」那个文件,且它 `import` 数为 0,verifier 可用 `loadPureModule` **真跑穷举**。
- 🔴 **刻意没有复用提交路径的取数/限张逻辑**:那条路在数量超限时是**抛错**的
  (「图片生成最多支持 1 张参考图」等三处),且判据吃的是**未截断**数组。
  若为共用而改成先 `slice` 再判,那三条硬闸会变成恒不成立的**死代码**,把「报错」悄悄变成「静默截断」
  —— 那是付费路径。**本批为此放弃了一次看似合理的复用**,提交路径一行未改。
  (该风险由两名对抗审查员独立指出,已复核属实。)
- 判定在 **provider 里整画布算一次**,不下放到节点内:节点内要拿上下游就得订阅 nodes/edges,
  而拖动时 store 逐帧 mutate `nodes`,200 个节点会各自逐帧重渲并各跑一次 O(E) 扫描,
  直接吃掉「200 节点 pan/zoom ≥50fps」那条预算。订阅带三道闸:`dragAnchor` 非空直接 return、
  `nodes`/`edges` 引用未变 return、集合内容相同则**保留旧 Set 引用**(它进了 context value 的 memo 依赖)。
- 角标挂 `NodeShell` 头部行 —— 唯一「所有节点类型 / 所有媒体状态 / 所有 zoom 档位都一定渲染」的横条;
  放正文要在图片与视频的多个分支各写一遍,新增分支必漏改,还会盖住 poster。
  icon-only + `title` + `sr-only` 全文 + `data-node-inputs="dirty"` 走查钩子。
- media-node 经 **context** 拿判定,不碰 store —— `verify-canvas-s6.mjs` 两条负向断言盯着该文件。

**闸门**:`tsc` 绿、`build` 绿(exit 0)、`generation-frontend` **163→182**(新增 19 条,含 13 条纯函数真跑穷举);
`s6 141` / `s3` / `s4` / `schema 119` / `runtime 544` / `intent 137` / `backend 88` 无回归;`reconcile` 绿。
「换图(两侧都无 generationId、仅 ossKey 变)必须 dirty」那条做过**负控验证**——
把该判据短路后 verifier 确实报红 2 条,再还原。

> ⚠️ **本批踩到并记下的一个坑**:`verify-canvas-s6.mjs` 的两条负向断言是**对整份源码做正则、不剥注释**的。
> 我在 media-node 的注释里照写了那个被禁的方法名紧跟左括号,直接把 s6 打红一次。
> 注释已改写并就地说明原因。**下个窗口在该文件写注释时注意这一点。**

🔴 **未做**:生产 UI 复验(角标是否真出现在正确的节点上、tooltip 文案、低 zoom 下的观感)。
本批证据全是离线闸门 + 代码复核。零成本可验(纯展示、不扣费)。

### #92 图片空态快捷 —— 🔴 提请裁决(见待裁决问题区 P1-Q4)

## P1 收尾 · 批 4:#182 @引用素材 + 顺带收口 #72/#94(状态=待审,**未做生产 UI 复验**)

### 核心裁定:@ 的第一动作恒为「建边」,插字只是给人看的第二步

契约层已经把话说死:**服务端从不读客户端传来的 `referenceNodeIds`,而是纯从 `doc.edges` 重算上游**,
再强制两者逐位全等。所以「只在提示词里写『图1』而不建边」= **什么引用都没建** ——
用户付全价拿回一张没垫图的图,静默、且要花钱才发现。

反过来,往提示词插字是**结构性安全**的:textarea 写的就是 `node.data.title`,
服务端 authoritativePrompt 吃的也是它,同一个归一化函数。唯一死路是「先存占位符、提交时再编译」——
那样服务端重算不到,每次必 409 `CANVAS_PROMPT_CHANGED`。**本批没走那条路。**

### 两处对抗审查查出的、已修正的设计缺陷

1. 🔴 **@历史 会把视频节点建成上游**。历史面板有视频页签,选中视频会建一条 `video → 生成节点` 的边;
   该边在画布层**零拦截**,要到提交时才报「当前生成节点不支持 video 输入」——
   而那是 **#185 确认花费弹窗之后**。已修:`onAddAsset` 只收 `item.type === "image"`,
   并顺带在满额时也事前挡住。
2. 🔴 **@ 触发字符没人清除**。「不 preventDefault + 插入 label」会在提示词里留下 `@图2`,
   而提示词**逐字送厂商、中间没有任何编译或清洗环节** —— 用户会为一个孤立的 `@` 付全价。
   已修:`applyMentionInsert` 按 `triggerIndex` 把触发字符一并吃掉;verifier 有断言钉死。

### 实现要点

- **两个入口**:①引用区的「+」瓦片(主入口,零 IME 风险,且 **0 张时那一行现在也渲染** ——
  正好兑现 #92 裁决理由里点名的「发现性提示应落在引用区 0 张时的空位」);
  ②提示词里输入 `@`(加速器,半角 `@` 与全角 `＠` 都认,组字期一律不认)。
- **编号绝不在选择器里另算**:新增 `previewImageReferencesAfterConnect`,内部复用
  `orderGenerationInputNodes` + `collectImageReferences`。理由是新边一律 append 到 `edges` 末尾,
  @ 进来的图**必然拿最大的 N**;若按候选列表下标编号,已有上游时 @ 一个节点会插成「图1」而实际是图2。
  `图${n}` 这个串在全仓仍只有一处产地,verifier 对三个文件断言其**零出现**。
- **超限事前禁用**:新增 lock kind `too_many_references` + 候选级 `disabled`+`reason`。
  此前超限只在 `buildGenerationIntent` 里抛,那已是 #185 确认之后 —— #186 立的纪律就是消灭这个。
- **浮层 portal 到 body**:面板在 React Flow 内会吃画布 transform 被缩放(zoom 可低至 0.4、
  节点选中宽仅 304px);且面板 inline↔停靠底部切换时整块换父重挂,内联列表的展开态与焦点会当场丢失。
- **文本/商品节点只建边、一个字不插**:它们的全文本来就会自动拼进提示词,
  再插一句字面记号只会给模型添乱。
- @历史 **复用 `CanvasHistoryPanel`,契约一字未改**(只换 `onAddAsset` 回调)。
  ⚠️ 但「复用 #64 即可」这个旧口径不准:#64 建的是**空的下游视频节点**,历史那条路只 `addNode`
  **不建任何边** —— @历史 要的是把两者拼起来的新胶水,小,但不是现成的。

### 顺带更正一处 CHECKLIST 笔误(事实更正,未动做/裁/期次)

#72 / #94 的复用件原写「复用 `reference-cache.ts`」——**那是商品链接解析缓存**
(`product_link_cache` 血统、按 `url_hash` 存 `ProductCard`、唯一消费者 `/api/studio/parse-reference`),
与画布参考图毫无关系。已改为真实复用件:`generation-input-order.ts` + `addNodeAndEdge` + `CanvasHistoryPanel`。
两项的「已引用」半边由 #44 承载、「怎么加」半边即本批,**与 #182 同批收口**。

**闸门**:`tsc` 绿、`build` 绿(exit 0)、`generation-frontend` **182→218**(新增 36 条,绝大多数是纯函数真跑穷举);
`backend 88` / `s6 141` / `s3` / `s4` / `schema 119` / `runtime 544` / `intent 137` 无回归;`reconcile` 绿。
编号一致性那两条做过**负控验证**:把 preview 的 append 改成 prepend 后 verifier 确实报红 2 条,再还原。

🔴 **未做**:生产 UI 复验(@ 唤起、IME 下的表现、候选灰置文案、浮层定位、@历史 真建上游节点)。
**其中 IME 那条只能靠真人验** —— 中文输入法下 `event.key` 到底给什么,离线闸门证明不了。

## P1 收尾 · 批 1/3/4 复验结果(2026-08-09,本地 dev 前台实测,**部分通过、部分被环境卡死**)

> **环境**:本地 dev `localhost:3000`(本工作区代码),真实 Chrome 前台,
> 自检 `{visibilityState:"visible", raf:"ok"}` —— **动画/媒体结论在此之后才算数**。
> 登录账号 ≠ 拥有生产 8 个画布的那个账号(它 canvases=0、图片历史 5 条、**视频历史 0 条**)。

### ✅ 已实测通过(9 条)

| 项 | 结果 |
|---|---|
| **#13** 引用区 0 张时渲染 +「+」瓦片 | ✅ 文案与瓦片均在,截图与 DOM 双证。**这条同时兑现了 #92 裁决理由里点名的发现性缺口** |
| **#14** 浮层打开、候选正确 | ✅ 自身不在列;header 文案正确;「从历史记录选一张图…」入口在 |
| **浮层 portal 到 body** | ✅ `menu.parentElement === document.body` —— 实现里最关键的一条设计得证 |
| **#15** @ 建**真边** | ✅ edges 0→1;**文本候选不插字**(提示词一字未变),toast「该节点的文字会自动加入本次提示词」——与设计裁定一致 |
| **#17** 已连接候选灰置 | ✅ 灰置且写明「已经连进这个节点了」,**事前禁用**而非点了才报错 |
| **#19** 输入 `@` 唤起浮层 | ✅ |
| **#20** IME 组字期不唤起 | ✅ `isComposing=true` 时 `@` 不弹浮层 —— **离线闸门证明不了的那条** |
| **#22** 浮层开着按 Enter | ✅ **未发起任何提交**,余额不变 —— 本批最要命的一条 |
| 画幅 6 项 + 端到端映射(P1-Q2a) | ✅ 面板恰好 6 项无越界档;9:16@1K 实际下发 `size: 768x1344`,与上游 sizeMap 逐字一致 |

另**顺带验到** #260 失败退款:上游 401 → 节点 failed → **退 5 分,余额始终 100**。

### 🔴 被环境卡死、未能复验(**已于 2026-08-09 深夜补完,结果见下一节**)

| 项 | 卡在哪 |
|---|---|
| **#82 裁剪全流程**(2-6) | 裁剪按钮 gate 在 `generation?.status==="completed"`,**本地拿不到产物** |
| **#43 正向**(8/9) | 同上,需节点有持久化 intent + 产物 |
| **#16 编号一致性** | 需两张带合法 ossKey 的图片节点;本地既生成不出、上传也进不来(见下) |
| **#23/#24 @历史** | 历史素材一加就把文档打成不可保存(见下);且该账号**视频历史为 0**,#24 无素材可验 |

**根因(已实证,均非本三批引入)**:
1. 🔴 **上游图片接口在本地 dev 返 401**:`POST https://api.hellobabygo.com/v1/images/generations` → 401。
   请求本身正确(model/size/prompt 都对),是**凭证在本机不可用**。⇒ 本地永远拿不到图片产物。
2. 🔴 **历史面板的素材会让画布存不下**:从历史加一张蓝图来源的图,PATCH 立刻
   `422 CANVAS_DOC_INVALID · 画布包含尚未完成服务端确认的上传对象 · rejected media objects: 1`,
   随后自动保存进入「保存已暂停」。**已在 `origin/main` 上用同样步骤复现** ——
   **是既有缺陷,不是批 1/3/4 引入的**。历史面板提供了画布根本持久化不了的素材。
   ⚠️ 这条值得单独立项:它同时意味着**生产上任何用户从历史面板加蓝图素材都会触发保存暂停**。

### 下个窗口补验的前提(**已解决**)

需要一个**能真正出图**的环境(可用的厂商凭证),或换到**已有画布产物的账号**上验。
拿到一张产物后,#82 / #43 正向 / #16 编号 四条可一次跑完。

## P1 收尾 · 四条补验的执行结果(2026-08-09 深夜,本地 dev 前台实测)

> **环境**:本地 dev `localhost:3000`(本工作区代码),真实 Chrome **前台**,
> 自检 `{visibilityState:"visible", raf:"ok"}`;账号 `2cca16b7-…`,起始余额 100 分。
> **模型开关已对齐生产**(`CANVAS_VIDEO_MODELS=happyhorse`,原本地是 `grok`)。

### 🔑 401 的真因:不是凭证过期,是本地压根没配

本地 `.env.local` **完全缺失整块 `VIDEO_PLATFORM_*`(12 个键)**,
`getVideoPlatformApiKey()` 三级回退全落空 ⇒ 裸请求 ⇒ 401。
已从生产机 `root@123.56.75.68` 的 release `.env.local` 取回
(`VIDEO_PLATFORM_*` 12 项 + `IMAGE_REFERENCE_ALLOWED_HOSTS` + `OPENAI_IMAGE_OSS_PREFIX`;
**刻意不取** `NODE_EXTRA_CA_CERTS` 等服务器专有项)。
`GET {base}/v1/models` 从本机返回 **200**,随后真出图成功 ⇒ 环境障碍解除。
⚠️ 该文件 git-ignored,备份在 `.env.local.bak-before-creds-44006ce`。

### ✅ #82 裁剪全流程 —— 6 条判据全过,**并揪出一个真缺陷**

| 判据 | 结果 |
|---|---|
| 1 产物动作行出现「裁剪」 | ✅ 行为 下载/全屏/裁剪/入库/推为参考 |
| 2 弹层打开、原图加载 | ✅ 图片真解码(948×1659),无 CORS `SecurityError` |
| 3 拖矩形 | ✅ 选区外压暗、选区内原样、边框可见、底部「已选 72% × 49% 的区域」 |
| 4 裁剪并新建节点 | ✅ 新节点位于源节点**右下**(1024,432 → 1384,652);`edges` 仍为 `[]`,**不与源节点连线** |
| 5 零扣费 | ✅ 余额 95→95、`generations` 1→1,均不动 |
| 6 选区过小 | ✅ 确认键 `disabled=true` + 「还没选区域:在图上按住并拖动即可」 |

🔴 **判据 4 里「标题「裁剪结果」」这条判据本身是错的,而代码照着它写出了缺陷。**
图片/视频节点**没有「显示标题」这回事** —— `data.title` 就是**逐字送厂商的提示词**:
`generation-controls.tsx` 提示词 textarea 是 `value={data.title}`,
同文件 @引用插字处自己写着「提示词就是 node.data.title,逐字送厂商」,
服务端 `generation-service.ts:409-418` 的**权威提示词**也正是
`[targetNode.data.title, ...上游文本节点.title]`。
实测:裁剪出的节点选中后,提示词框里赫然是「裁剪结果」四个字。

**同类共三处,全部已修(只给 `media`,提示词留空走 placeholder,与拖文件/上传/素材库建节点一致)**:

| 处 | 原值 | 引入者 | 是否已上生产 |
|---|---|---|---|
| `generation-controls.tsx` 裁剪新建节点 | `title: "裁剪结果"` | 批 1(#82) | 否(本工作区未合并) |
| 同文件 @历史建上游节点 | `title: "引用素材"` | 批 4(#182) | 否 |
| 同文件「推为参考」建视频节点 | `title: "以图生视频"` | **`5b53a95`(#64)** | 🔴 **是** —— `git merge-base --is-ancestor 5b53a95 3dee031` 成立 |

🔴 **第三处已在生产运行**:任何用户对一张图点「推为参考」,新建的视频节点提示词框里就预置着
「以图生视频」五个字;一按生成就以这五个字当提示词跑一次 **450 积分**的视频。
不是静默扣费(用户看得见输入框、需自己点),但那串字读起来像模式说明、不像提示词。
**修复要真正生效需要发版。**
全仓复扫已确认无第四处(所有建节点处只传 `media`;`group-frame.ts` 的 `data.label` 是 RF 展示节点,不是域节点)。
修复后复测:新裁剪节点 `title: null`、提示词框为空且显示 placeholder;
@历史建出的节点在候选列表里显示为「未命名图片节点」,与上传节点待遇一致。

### ✅ #16 @引用编号一致性 —— 通过

场景:视频节点(HappyHorse / 图生视频,`maxImages=9`)先用拖把手连图 A,再用 `@` 连裁剪节点 B。

- 提示词从「让它转起来 @」变为「让它转起来 **图2**」—— **不是「图1」** ✅
- 触发用的那个 `@` **已消失**(判据 19)✅
- `edges` 真增第二条(`node_Er5kfaPIU9LG->node_13JcQTTKRsJQ`)✅
- 引用区变「· 2 张参考图」,两个缩略图角标分别为 **图1→苹果原图 / 图2→裁剪图**,与插入文字指代一致 ✅
- toast「已引用」✅

**顺带复证**:#17(图片节点连满 1 张后其余图片候选灰置,理由「本节点最多用 1 张参考图;先断开现有的那条图片连线」,**事前禁用**)、
#18(文生视频模式下图片候选整体灰置,「文生视频不使用图片输入;把模式改成图生视频后才能引用图片」)、
空图片节点单独灰置为「这个图片节点还没有内容」(不会建出必报错的上游)。

### ⚠️ #43「输入已更新」角标 —— **角标首次在真实 UI 亮起**,判据 9/10 全过,判据 8 只做到一半

| 判据 | 结果 |
|---|---|
| 7 无快照 ⇒ 不挂角标 | ✅ 视频节点连了两条边(输入变了两次)、从未提交 ⇒ 全程 `dirtyBadges=0` |
| **角标本体** | ✅ `[data-node-inputs="dirty"]` 出现在节点头部,琥珀色 ⚠;`title` 与 `sr-only` 均为**「输入已更新:上游内容在这次产物之后变过了,重新生成即可同步」**,与判据逐字一致 |
| 9 改上游**文本** | ✅ 改文本节点的字 ⇒ 下游立刻挂角标(变的只有 `textSha256`);**把文字改回原样 ⇒ 角标消失** —— 反向也成立,证明判定是内容哈希而非「碰过就算」 |
| 10 拖动 / 改本节点标题 | ✅ 改本节点自己的提示词 + 拖动节点(0,0 → -128,48),`dirtyCount` 全程 0 |
| **8 上游图片换一张** | ⚠️ **只验到「输入集合变化(`added`)⇒ 角标亮」**;「同一上游节点换一张图 ⇒ `ossKey` 变 ⇒ `replaced`/`regenerated`」**未能实测** |

**判据 8 卡在哪(外部条件,非代码问题)**:该分支要求下游节点的**持久化快照里已经含一个图片输入**,
即需要一次**成功的、带图片上游的生成**。2026-08-09 深夜厂商 `gpt-image-2` **连续 3 次失败**
(2 次 `No available channel for model gpt-image-2`、1 次 `图片生成超过最大等待时间 300000ms`),
无法构造该前置。**该分支目前只有离线穷举覆盖**,待厂商恢复后补。

**另外两条顺带得证**:
- **#260 失败退款**:3 笔失败全部自动退款,余额始终自洽(100 起、净扣 5 = 唯一那笔成功)。
- **未绑定 intent 的过渡态**:两次提交后都短暂出现「上次提交没能确认送达,服务端目前查不到这笔任务…
  此时再提交会是一次全新的扣费,不是恢复」+「放弃这次提交 / 等待幂等核对」,
  随后由对账收敛为 `bound`,**没有二次扣费** —— 设计按预期生效。
- **#44 内容节点删除二次确认**:两次触发均弹「删除该节点?将同时移除该节点及相关连线」/
  「删除所选? 将删除 1 个节点(连带相关连线),此操作可撤销(Ctrl+Z)」。

### ✅ #23 @历史建上游节点 —— 通过 / 🔴 #24 —— **该账号无素材可验**

- **#23**:从历史面板选「生成」来源的那张图 ⇒ 新图片节点建在目标节点**左侧同一水平线**
  (新节点 x=-56,y=360;目标 x=304,y=360),并自动连成**上游**边;`title: null`(上面那处修复生效);
  保存状态保持「已保存」✅
- 🔴 **#24 无法执行**:该账号**视频历史为 0**(历史面板页签实测显示「图片 6 / 视频 0 / 音频 0」),
  「切到视频页签再选一个」这一步**没有可选项**。需要一个有视频产物的账号,或先跑一次视频生成
  (5 秒 450 积分,本账号余额 95,跑不起)。**仍挂着,不要当作已过。**

## 两个影响生产的既有缺陷(本轮查实)· **甲已在生产关闭;🔴 乙已修但在生产仍是原样**

> 🔴 **别把这一节读成「两个都关了」。** 只有**甲**(保存闸)靠 20260810 迁移在生产真关掉并复验过。
> **乙**(快图 worker)的修复是纯代码、在未 push 的分支上,而且**合 main + 蓝绿发版也修不好它** ——
> 它的执行体是 crontab 每分钟打的 `/var/www/tiktok-ai-mcn/run-image-worker.sh` → `127.0.0.1:3000`(老应用目录),
> 而蓝绿发版只建新 release 目录 + 新端口 + 切 nginx,老目录一行不动。详见 HANDOFF_P1_FINISH §四 A2。

> **用户裁决(2026-08-10)**:缺陷甲走**方案 B**(放宽保存闸白名单);缺陷乙一并修;
> 「推为参考」那处生产缺陷的修复**攒着与批 5/批 6 一起发版**,不单独发。
>
> **最终落地**:`11a951f` —— 迁移 `20260810_canvas_media_readiness_history_assets.sql`
> 与 `image-generation-worker.ts` 两处查询加 `.or(source.is.null,source.neq.canvas)`。
> **2026-08-10 由用户授权、AI 在 Supabase dashboard 执行,返回 `Success. No rows returned`
> (含末尾 DO 后置断言)。**
>
> 🔴 **初版(`5f0429a`)是半个修复,被执行前的对抗审查拦下 —— 这段务必读**:
> 初版只放行蓝图,并抬头断言「generations 已在白名单」。**该断言是错的**:
> 闸门分支③只比 `output_oss_key`,而**该列只有画布自己的行会写**
> (20260716 三条约束均以 `action_id IS NULL OR …` 开头)。历史面板却是从
> `output_url/result_url/video_url/image_url` 换算 objectKey。⇒ 幻灯片
> `videos/slideshow/<uid>/…`、拼装 `videos/assembly/<uid>/…`、统一视频网关的
> `quick-gen/<uid>/…` 这批**所有者路径布局的非画布产物**,面板列得出、能加进画布,
> 却一条分支都不命中,**加了照样 422**。生产实测该形状的行有 5 条。
> **若按初版执行并只走蓝图复验,会得到假绿。**
>
> **审查过程**:5 镜头对抗审查 → 17 条发现 → 逐条反驳杀掉 16 条 → 存活 1 条 major(即上条)。
> 另有三处**审查自己判错、由生产实测纠正**的,记下来防下个窗口重蹈:
...
>
> **终版的四处改动**(相对初版):放行面扩到「面板给得出的素材」= 蓝图 + 自己已完成生成的
> URL 列;**改内联、不再新增 helper 函数**(消掉依赖顺序/回滚顺序/健康检查登记三个坑);
> 匹配右端一并锚死(`right(url, len+1) = '/'||key`);补末尾 DO 后置断言。
>
> ✅ **「本地无法预演」这个限制已解除**:新增 `scripts/verify-canvas-readiness-migration.mjs`
> —— PGlite 起真 Postgres,按生产列结构建最小 schema、执行迁移原文、灌真实形状的行跑
> **16 条正反用例**(含幻灯片缺口、左右边界、原三条分支无回归),**全绿**,
> 不连生产、不需凭证。以后画布这类迁移都该先过它。
>
> ✅ **生产复验(2026-08-10,执行后)**:
> | 用例 | 结果 |
> |---|---|
> | 蓝图素材(最初肇事的那张鞋) | `ok:true` |
> | 幻灯片视频(真实生产行) | `ok:true` ← 初版会整条漏掉 |
> | 从未被引用的 key | 仍拒 `not_ready` |
> | 他人 uid 的 key | 仍拒 `not_owned` |
> | 前缀 key(右边界) | 仍拒 `not_ready` |
>
> **UI 端到端**:历史面板加一张蓝图鞋图 ⇒ `rev 32→33`、节点 9→10、
> 蓝图素材节点 1→2、状态**「已保存」**(不再是「保存已暂停」)。缺陷关闭。
>
> **回滚办法**(内联后只有一句):把 `20260801_canvas_upload_registry.sql:684-846` 的
> `assert_canvas_media_keys_ready_v1` 原版原样 `CREATE OR REPLACE` 回去即可,无顺序陷阱。
> 执行前已核对生产函数体与该文件**逐字节一致**(仅 CRLF 差异),零漂移。

### 缺陷甲 · 历史面板有 2/3 的来源「一加进画布就存不下」

**表现(已精确复现)**:历史面板加一张**蓝图**来源的图 ⇒ `PATCH /api/canvas/{id}` **422**
⇒ 顶部红条「自动保存已暂停以保护当前修改。请保持此页面打开,当前修改仍保留在本页。」

**根因(读 RPC 定义得出,非猜测)**:保存闸 `assert_canvas_media_keys_ready_v1`
(`supabase/migrations/20260801_canvas_upload_registry.sql`)只接受三类 object key:
①`canvas_upload_reservations` 里 `status='ready'` 的;②**当前文档里已经存在**的(存量兼容);
③`generations` 里 `status='completed'` 的 `output_oss_key`。
而历史面板(`/api/canvas/history`)供的是 **`generations` / `products` / `blueprints` 三个来源** ——
**后两个一个都不在白名单里**。⇒ 不是「蓝图」独有,是 **products 与 blueprints 整两类**。
本测试账号的图片历史 6 条里,**5 条是这种加了就存不下的**。

🔴 **暂停是粘性的,且 UI 里没有恢复入口**:实测删掉肇事节点(9→8)后仍是「保存已暂停」,
再等 15 秒不变,`Ctrl+Z` 也不解除,面板上找不到任何「重试/恢复保存」按钮。
**唯一已知解法 = 删掉肇事节点 + 刷新页面**(实测刷新后回到「已保存」,8 个节点由影子副本兜住,没丢东西)。

**影响面**:画布已对所有登录用户开放;任何人从历史面板挑一张蓝图/商品素材,
本次会话的自动保存就停了,且不会自己好。

**两条路,须裁决**(AI 不擅自选,触铁律 10 与迁移纪律):
| | 方案 | 代价 |
|---|---|---|
| A | **收窄历史面板**:画布侧只列 `generations` 来源(或把另两类置灰并写明原因) | 纯前端,无迁移,当天可上;但等于砍掉「从蓝图/商品挑素材」这个能力 |
| B | **放宽保存闸白名单**:让 RPC 也认 `products`/`blueprints` 名下属于本人的 key | 能力保全;但要**改 RPC = 生产迁移**(须用户在 Supabase dashboard 执行),且要想清楚归属校验怎么写才不放大权限面 |

### 缺陷乙 · 快图 worker 会把**画布**的生成认领走,且它的退款不写画布对账列

**证据链**:
1. 本轮一笔**画布**生成(`source=canvas`)以 `图片生成超过最大等待时间 300000ms` 失败 ——
   这句话**全仓只存在于 `src/lib/image-generation-worker.ts`**(:900/:1557/:1632)。
2. 对应的退款流水描述是「**图片生成失败自动退款 - GPT Image 2 (task_…)**」
   (`image-generation-worker.ts:510`,ledger scope `quick-image`),
   **不是**画布自己那句「Canvas failure refund」(`20260719_canvas_p1_reconciliation.sql:624`)。
3. `processImageGenerationQueue` 拉队列的条件只有
   `.in("status",["processing","pending"]).eq("type","image")` —— **没有 `source` 过滤**
   (`image-generation-worker.ts:1880-1884`)。
4. 生产 crontab:`* * * * * /var/www/tiktok-ai-mcn/run-image-worker.sh`,
   **每分钟**打 `127.0.0.1:3000/api/cron/process-image-generation`;`ss -ltnp` 确认 3000 在听
   (pm2 `tiktok-ai-mcn`,即**老应用目录**,不是当前 release 的 3015)。

**后果**:
- 画布有自己的对账车道(`stargaze-canvas-reconciler`),现在**同一行可能被两条车道各自处理**;
- 走快图车道退的款**只写 `credit_transactions`,不回写 `generations.credits_refunded`** ——
  实测那一行 `credits_refunded=0` 而余额确实退回。**用户的钱是对的,但画布对账口径会少记退款。**
- 附带:该 worker 跑的是**老应用目录的旧代码**,与当前 release 不同源。

⚠️ **本条只观察到一次,未观察到重复退款或用户被多扣**,不要外推成资损。
建议裁决点:①`processImageGenerationQueue` 是否应加 `.neq("source","canvas")`;
②快图退款路径是否应一并回写 `credits_refunded`;③cron 指向 3000 是否是发布流程的遗留。

## 🔴 原待复验清单(逐条判据,保留;上面是执行结果)

> **为什么必须本地 dev 验,不能对生产验**:批 1/3/4 的代码**没有部署**。
> 线上是 `3dee031`,这三批只是本工作区的本地 commit。对生产验只会验到旧代码。
>
> **起法**(本工作区已备好 `.claude/launch.json`,`autoPort: true`):
> ⚠️ **3100 端口被兄弟工作区 `canvas-p1-acceptance` 的 dev 占着且已无响应(HTTP 000)**,
> 那是**批 0 之前的旧代码**,连上去等于验错东西。**别用 3100。** 本轮实际起在 **3000**。
>
> **两个前置(只有真人能做)**:
> 1. **登录**:dev 用的是同一套 Supabase,`/canvas` 匿名会被 middleware 挡回登录页。
>    AI **不能代输密码**,须用户自己登录。
> 2. **把浏览器窗口调到前台**:本轮两次自检都是 `{visibilityState:"hidden", raf:"NO-FRAME"}`。
>    按 §七,`NO-FRAME` 下**动画/媒体相关结论一律作废**(`<video>` 会卡在 `readyState 0`)。
>    DOM/布局断言不受影响,但下面带 🎞 的项必须在前台才算数。
>
> **开工第一条命令永远是那句自检**,返回 `NO-FRAME` 就停下要求置前,别硬验。

### 批 1 · #82 裁剪(零扣费,可放心验)

| # | 验什么 | 通过判据 |
|---|---|---|
| 1 | 选中一个**已有产物**的图片节点,产物动作行是否出现「裁剪」 | 该行为 下载/全屏/裁剪 三键;只读态下不出现 |
| 2 | 🎞 点「裁剪」弹层是否打开、原图是否加载出来 | 图显示;**若图裂了大概率是 CORS**,查 Console 有无 `SecurityError` |
| 3 | 🎞 在图上拖一个矩形 | 选区外压暗、选区内原样、边框可见;底部显示「已选 N% × M%」 |
| 4 | 点「裁剪并新建节点」 | **画布上出现一个新图片节点**(标题「裁剪结果」),位置在源节点右下;**不与源节点连线**(这是刻意的) |
| 5 | 裁剪是否真的零扣费 | 操作前后余额不变;`generations` 不新增行 |
| 6 | 选区过小时 | 确认键灰置,提示「还没选区域」 |

### 批 3 · #43「输入已更新」角标(纯展示,零扣费)

| # | 验什么 | 通过判据 |
|---|---|---|
| 7 | 打开一个**存量画布**(生产上已有 8 个文档) | **所有节点都不该有角标** —— 老文档没有提交快照,判定必须是 not dirty。若一片橙色感叹号,说明「缺快照=not dirty」那条被写反了 |
| 8 | 拿一条 图→视频 的链:先在视频节点跑一次生成,再把上游图片节点**换一张图**(重新生成 / 上传 / #82 裁剪结果连过去) | 视频节点头部出现橙色 ⚠ 角标,悬停文案为「输入已更新:上游内容在这次产物之后变过了,重新生成即可同步」 |
| 9 | 改上游**文本节点**的文字 | 下游同样挂角标(判据含 `textSha256`) |
| 10 | 拖动节点 / 改本节点标题 | **不得**挂角标(纯位置与自身变化不算输入变化) |
| 11 | 低 zoom(<0.4)下 | 角标仍在(它不解码媒体,刻意保留) |
| 12 | 只读态(开第二个标签页占写者锁) | 角标照常显示 —— 它是纯派生的,不依赖写权限 |

### 批 4 · #182 @引用素材(建边会改文档,但零扣费)

| # | 验什么 | 通过判据 |
|---|---|---|
| 13 | 选中一个图片/视频节点,引用区在 **0 张参考图**时是否也显示 | 显示一行:「引用区 · 0 张参考图（连一个图片节点进来即为图生图…）」+ 一个虚线「+」瓦片 |
| 14 | 点「+」 | 浮层出现,列出画布上的文本/商品/图片节点;**视频节点不在列**;自身不在列 |
| 15 | 选一个图片候选 | **画布上真的多出一条连线**(上游→本节点);提示词框里插入「图N」;引用区多一个缩略图 |
| 16 | 🔴 **编号一致性**(本批唯一会花钱的失败模式) | 先连一张 A,再用 @ 连 B ⇒ 插入的必须是「**图2**」而不是「图1」;引用区里 B 的角标也必须是 2 |
| 17 | 图片节点已有 1 张参考图时再开选择器 | 其余图片候选**灰置**并写明「本节点最多用 1 张参考图…」——**不是点了才报错** |
| 18 | 视频节点切「文生视频」后开选择器 | 图片候选整体灰置,理由为「文生视频不使用图片输入…」 |
| 19 | 在提示词框里输入 `@` | 浮层弹出;**选中候选后原来那个 `@` 必须消失**(留下孤立 `@` 会逐字送进厂商提示词) |
| 20 | 🔴 **IME**(只能真人验,离线闸门证明不了) | 切中文输入法,打字过程中出现的 `@`(含全角 `＠`)**不得**在组字期弹浮层;组字期按 Enter 是选词,**不得**触发生成 |
| 21 | 浮层开着按 Ctrl+Enter | 仍然走生成(浮层不吞它) |
| 22 | 浮层开着按 Enter | 选中候选,**绝不能发起一次付费提交** |
| 23 | 「从历史记录选一张图…」 | 打开历史面板;选一张**图片** ⇒ 在本节点**左侧**建一个新图片节点并自动连线 |
| 24 | 🔴 历史面板切到**视频**页签再选一个 | 必须被拒:toast「只能引用图片素材」,**不建节点也不建边** |
| 25 | 低 zoom / 面板停靠底部时开浮层 | 浮层不随画布缩放、不被面板换父重挂弄丢(它 portal 到 body) |
...
**开工时已知的硬约束**(逐条在实现里守住,验收时逐条回填结果):

| # | 约束 | 依据 |
|---|---|---|
| 1 | 防重复解析:该路由对用户零扣费但**对我方有厂商成本**(豆包视觉 / Qwen 降级),不得每次渲染都跑;「重新解析」必须是显式入口 | P1-Q5 ⚠️① |
| 2 | verifier 断言写成「画布对**计费类**路由引用数 = 0」,**不能**写「非 `/api/canvas/*` 引用数 = 0」——#64 入库已在调 `/api/studio/library` 且已在生产 | P1-Q5 ⚠️②;`generation-controls.tsx:348` |
| 3 | 画布文档**只存 OSS object key**,禁存签名 URL;而 `analyze-product` 只收 `startsWith("http")` 的 `imageUrls` ⇒ 换算只能发生在**运行时**,不得落盘 | 铁律 3;`analyze-product/route.ts:23-27` |
