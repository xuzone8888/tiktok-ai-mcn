...
- **下一步(S3 收尾三件套,均待用户)**:①Supabase dashboard 执行 **20260702_drop_link_video.sql + 20260703_recipes.sql**(顺序无关,后者执行后用户配方全链自动激活);②**备 20 条真实爆款 mp4** 跑 `node scripts/deconstruct-benchmark.mjs <目录>`(dev:3100 需在跑)——自动三线+人工抽查达标后把 NEXT_PUBLIC_ENABLE_DECONSTRUCT=1 加进生产 env 开门B UI;③dev:3100 人工走查 S3 新 UI(拼装腿出片/配方选择与存为配方 diff/门B 上传→报告,flag 本机已开);之后裁决是否 push main 上生产(自动部署链在,push 即上线)。**S4(配方库收编模板中心+photo post 直发+video-batch 大页消解)建议新窗口开工**
