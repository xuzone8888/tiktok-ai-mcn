...
- 🆕 **2026-08-09 · 四项裁决 + CHECKLIST 改判落笔**:
  ...
  - 🔴 **待做:生产 UI 复验**。本批全部改动至今只过离线闸门 + 公网字符串证据,**没有在真实浏览器里渲染过**。零成本可验项:引用区缩略图与序号、全屏预览、灰置控件解锁指引、参数悬停文案、刷新状态按钮;「入库」需一笔已完成产物(用现有的,不新生成)。**另需重测 1366×768 面板高度**——本批往面板加了约 110px(引用区/解锁指引/入库行),`GENERATION_DOCK_MAX_HEIGHT_RATIO=0.75` 当初的依据「容得下 389px 视频面板」已被自己的改动作废
