...
- **S2.11 已知问题(用户线上验收发现,根因已坐实,待新窗口修复)**:多镜 AI 生成(3 图→3 镜 grok)逐镜生成全部成功(扣 15 分,scene 成片已在 generations),**stitch 拼接失败,任务显示裸 "fetch failed"**。根因双实锤:①生产日志 `[Studio AiGen Stitch] error: TypeError: fetch failed`(01:47);②生产 env 的 MAC_WORKER_URL 探测 000——**Mac worker 离线**,stitch 路由唯一通道断了。**修复方案(已定)**:仿 generate-slideshow 的双通道模式,stitch 路由加"worker 不可达→服务器本地 ffmpeg 拼接"回退(生产服务器有 ffmpeg 6.1.1+ffprobe,slideshow 本地回退一直在用;ffmpeg 参数参照 mac-ffmpeg-worker/stitch.js:scale+pad 统一分辨率/fps30/anullsrc 补静音/concat/loudnorm);顺带把 worker/本地双败时的错误文案改友好(不再裸透传 fetch failed),客户端执行器 stitch 失败信息同步格式化。**验证路径:线上重试该失败 job——scenes 已 completed 零新扣费,直接重进 stitch**(retryTask→pending→执行器跳过已完成镜→stitch)
- **下一步:**①新窗口修 S2.11(本地 ffmpeg 拼接回退)→push 自动部署→线上重试失败 job 验证;②dashboard 执行 20260702_drop_link_video.sql(线上已无 link-video 代码,顺序已满足);③S3 = 拼装腿+爆款拆解(20 条基准)+批量矩阵+存为配方(BLUEPRINT §七)
