...
- **下一步:**①(可选)用户线上对原失败批次点「重试失败 1 条」确认体感(代码路径已脚本实锤);②用户经 dashboard 执行 20260702_drop_link_video.sql(线上已无 link-video 代码,顺序已满足);③S3 = 拼装腿+爆款拆解(20 条真实爆款基准门槛,达标才开 UI)+批量矩阵+存为配方(读 BLUEPRINT_PIPELINE_PLAN §五/§七;建议新窗口开工)
