...
- **批 0-4 交付**:#82 裁剪(6/6 判据全过)、#43 dirty 角标(判据 7/9/10 过)、#182 @引用素材
  (含 #16 编号一致性、#23 @历史建上游节点)、#72/#94 收口;#78/#81/#85/#92 经裁决改判延 P2。
- 🔑 **解除了一个长期误判**:此前所有窗口都认为本地出图失败是「厂商凭证过期」。
  **真因是本地 `.env.local` 整块缺失 `VIDEO_PLATFORM_*`(12 个键)**,三级回退全落空才 401。
  已从生产 release 取回(排除服务器专有项),`{base}/v1/models` 本机返 200,随后真出图成功。
...
- 🔴 **执行生产 DDL 前的对抗审查推翻了初版的承重前提**(这条最值得记):
  ...
  ⇒ 幻灯片 `videos/slideshow/<uid>/…`、拼装、quick-gen 那批产物**加进画布照样 422**(生产实测 5 条)。
  **按初版执行并只走蓝图复验会得到假绿。** 审查 17 条发现、反驳杀掉 16 条、存活这 1 条 major。
- **三处「审查判错、生产实测纠正」**:`to_regclass('public.products')`=NULL(表确实不存在,**不能引用**)、
  `generations` **无 `output_urls` 列**(照建议加会运行时炸)、画布行**会**同时写 URL 列。
- 🔴 **线上现存一个已修但未发版的缺陷**:图片/视频节点没有「显示标题」,`data.title` 就是逐字送厂商的提示词。
  「推为参考」那处(#64/`5b53a95`)**已随 `3dee031` 上生产** —— 建出的视频节点带着「以图生视频」当提示词,
  一按生成 450 积分。三处均已在本分支修好,**发版才生效**。
- **仍开着的**:#43 判据 8 的 `replaced`/`regenerated` 分支(厂商 gpt-image-2 连续无通道,未能构造前置)、
  #24 历史面板视频页签(测试账号视频历史为 0)。详见 HANDOFF_P1_FINISH §四。
- 🆕 **2026-08-09 · 四项裁决 + CHECKLIST 改判落笔**:
  ...
  - ✅ **已发版(2026-08-09)**:PR #34 合入 main → `33ba71d0d2a333b16ea4ba2b664cd61ced436ddb`,蓝绿切到端口 **3014**,`DEPLOY_RC=0`,BUILD_ID 门通过。回滚包 `canvas-rollback-20260809T050329Z-port-3013-991403`;`d16620f`/3013 与 `abc29ac`/3012 两个回滚位仍在线。发版脚本存档 `/var/backups/stargaze-canvas/deploy-33ba71d-20260809T050329Z.sh`。**本批零迁移**,合 PR 即代码就绪,无需手工 SQL
  - **新代码已在公网证实真在跑**:拉 `https://toryxai.com/_next/static/chunks/app/(canvas)/canvas/page-4f99d207765a7c83.js`(HTTP 200/317KB),「为什么现在不能生成」「引用区」「推为参考」「刷新状态」「余额可能不够」「本次为大额消耗」六个本批新字符串全部命中 —— 不是只看进程起没起
  - 🔴 **发版过程中打挂过整站约 15 分钟(构建 OOM)**。根因两条叠加:①**release pm2 进程只加不减**(事故时 9 个共 1263MB,上次成功构建时仅 1007MB);②**`vm.swappiness=0`** 使 2G swap 全程 0 使用、内核宁可 OOM-kill 也不换页。**发版脚本「构建失败则在任何流量变更前 abort」救了场**——没发生蓝绿切换,内存一放出来站点自愈,零回滚。处置=`pm2 stop` 三个 3 代以前的 release 进程(`fea0bcb`/3007、`e77d4df`/3010、`a24a4e3`/3011)释放 423MB,available 1664→2036MB,重跑一次过。**下次发版前置见 HANDOFF「负三之零」**
  - ⚠️ **一个被推翻的判断**:此前(含本轮早些时候)把「清理老 release」当**磁盘**问题——错的。磁盘从来不紧张(事故时余 10G),**真正的紧箍咒是内存**
  - ✅ **生产 UI 复验已完成(2026-08-09,线上 `33ba71d`,前台可见标签)**:#84 全屏预览(开/Close/真实 Escape/卸载/body 恢复/视频 `readyState 4` 播放)、#44+#72+#94 引用区(缩略图真加载 + 推为参考建的新节点立刻显示图1)、#187 四参数文案三处齐全、#186 只读分支文案正确、#51③ 只读隐藏可写显示、#64 入库(服务端确实更新,走新增的 `generationIds` 匹配键)、#64 推为参考(建节点+连线+持久化 rev+1)、#250 删除二次确认顺带验到。详表见 P0 看板
  - 🔴 **复验最大教训**:最初在 **hidden 标签页**里做,Chrome 停发 `rAF` → 动画永不结束 → Radix 弹层不卸载 → `body{pointer-events:none}` 常驻,**与真实的阻断缺陷现象完全一致**,两次「干净复现」后差点建议回滚一个正常功能。已写成可执行自检进 HANDOFF §七
  - ⚠️ **仍在的非阻断摩擦**:1352×642 下视频面板溢出停靠位 66px(`485 vs 419`),「下载/全屏/去发布」整行落在切掉的部分。比改造前(溢出 95px)有改善;继续抬比例会让停靠位吃掉大半画布,**不再调**
  - 🔴 **待做(旧口径,保留对照)**:生产 UI 复验。本批全部改动至今只过离线闸门 + 公网字符串证据,**没有在真实浏览器里渲染过**。零成本可验项:引用区缩略图与序号、全屏预览、灰置控件解锁指引、参数悬停文案、刷新状态按钮;「入库」需一笔已完成产物(用现有的,不新生成)。**另需重测 1366×768 面板高度**——本批往面板加了约 110px(引用区/解锁指引/入库行),`GENERATION_DOCK_MAX_HEIGHT_RATIO=0.75` 当初的依据「容得下 389px 视频面板」已被自己的改动作废
  - ⚠️ **#185 + #188 的连带后果(待用户定夺)**:阈值改造后,余额 18459 时 450 分的视频**两条阈值都不触发**,于是提示词框里一次误触 Ctrl+Enter 就直接扣 450 分、零确认。按钮那条路本来就是「点了就扣」,但文本框里的 Ctrl+Enter 不是刻意动作
...
- ✅ **卡住的积分已于 2026-08-09 全部结清**:3 笔(视频 329a5399/02e20c34、图片 9848fcb4)走 verified_no_task_refund,各退 5,余额 18459→18474,生产已无 pending 行。**最大收获是 R2-Q2 那个让人工裁决 RPC 支持图片行的迁移至此才第一次在真实图片行上跑通**
