...
- **下一步:**执行 S3.5(存为配方:recipes 表+脱敏 diff+配方库+实例化)。待用户手动:①(可选)线上对原失败批次点「重试失败 1 条」体感确认;②Supabase dashboard 执行 20260702_drop_link_video.sql;③**为基准备 20 条真实爆款 mp4(本地文件夹即可)**——达标后把 NEXT_PUBLIC_ENABLE_DECONSTRUCT=1 加进生产 env 开 UI
