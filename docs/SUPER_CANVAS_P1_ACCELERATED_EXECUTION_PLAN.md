...
### 永久检查点（2026-07-28 16:59 CST，部署状态已固化）

- 固化前再次确认 candidate PID `1091043`、online/restart 0，active/rollback Nginx SHA 分别仍为 `fef1d150...e59fa4` / `1ddb35f5...dc354`，外部 Canvas 为 307、公开模型为 `200 / 48,912 bytes`，`nginx -t` 成功。生产数据库固定 postflight 于 `2026-07-28T08:59:16.416462Z` 再次得到 18/18、mismatch 空、四项业务计数不变、最近活动/会话/负余额/重复 anchor/submission_unknown 全为 0。
- 已执行 `pm2 save` 并只输出非敏感字段核对 `/root/.pm2/dump.pm2`：`tiktok-ai-mcn` 固定 `/var/www/tiktok-ai-mcn / 3000`，`tiktok-ai-mcn-candidate` 固定精确 release / `3002`，`webhook` 与 `okspeak-proxy / 8788` 均仍存在。重启恢复因此保留新正式路径和旧版本即时回滚路径；未输出任何 PM2 环境值。
...
### 永久 post-edit / pre-100-node 检查点（2026-07-28 17:14 CST）

- 产品 UI 唯一文本框输入 `Canvas P1 production smoke 2026-07-28` 并失焦；等待自动保存后只读数据库为 `rev=2 / doc_bytes=287 / nodes=1`，目标节点 `data.title` 逐字匹配。rev 从 0 到 2 包含创建后 route/writer adoption 与本次编辑收敛，没有重复 Canvas：全库仍仅该一行。
- generation 仍为 0，目标 ledger 仍为 1。再次重新导航同一正式 URL 后输入值逐字恢复、节点数仍 1、无恢复入口，证明创建、编辑、自动保存、刷新主链已在新正式应用和迁移后数据库端到端通过。
- **下一唯一动作**：保持同一 writer/Canvas，通过产品“添加文本节点”入口在一个紧凑 UI 操作中再添加恰好 99 个节点，使总数为 100；等自动保存只发生一个最终稳定提交后，从 UI/数据库/请求观测共同记录节点数、rev、doc_bytes 与单次保存时延。若最终不是精确 100 或出现 5xx/恢复态，停止后续付费样本。

### 永久 post-100-node / pre-timing 检查点（2026-07-28 17:22 CST）

- 首次批量 UI 循环在工具 60 秒上限处中断并重置浏览器控制内核；没有把超时当作失败或猜测继续。重新接管同一正式标签后从可见 UI 精确计得 74 节点，再通过同一个可见“添加文本节点”按钮补 26，最终 UI 精确 100 且无恢复入口。没有直接调用 Canvas API、SQL INSERT 或修改浏览器存储。
...
### 永久 post-image-failure / pre-slideshow 检查点（2026-07-28 17:47 CST）

- `/quick-gen` 固定样本、固定提示词、1K/5 积分的“生成 图片”按钮只点击一次，提交时间 `2026-07-28T09:36:34.396Z`。处理中按钮立即禁用，余额由 100 变 95；数据库只产生唯一 generation `889f7e5d-7013-401a-b8f1-ca58c3c548ee`、唯一 `-5` consume，证明连续/重复点击没有造成第二任务或第二扣费。
- 后台 worker 按既有每分钟调度对唯一任务做 4 次重试，均收到上游 `HTTP 503 / No available channel for model gpt-image-2 under group Codex (distributor)`；第 4 次在 300,000ms 最大等待闸门后于 `2026-07-28T09:44:05.738Z` 确定性转为 failed。退款随后自动完成：追加唯一 `+5` refund，profile credits 恢复 100，账本连续为 `0→100→95→100`，`refund_pending=false`，没有人工修表或重提。
- 产品 UI 已同步显示余额 100、“生成失败”和精确最大等待错误；输入和按钮重新可用，但本任务禁止点击重试。供应商只读 `/v1/models` 返回 HTTP 200、唯一模型且精确包含 `gpt-image-2`，说明 key、域名、连通性和模型名均正确；真正的图片执行渠道在供应商分发组内不可用，不能通过改本仓代码、猜测替代模型或改生产密钥安全解除。
