...
- **接口分歧走裁决**:data 与 shell 可就接口自由讨论,但**结论必须落成文字**——落到 `src/lib/canvas/schema.ts`(代码即契约,首选)、看板对应任务的备注、或总纲 §四 ADR(新增条目);口头/聊天结论不算数,下个窗口看不到就会返工。自决不了的分歧由审核窗(集成负责人)+ 技术负责人裁决,裁决同样落上述三处之一。
