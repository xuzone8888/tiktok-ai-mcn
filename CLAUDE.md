...
> **P0 已完成并上线;R2 已于 2026-08-08 晚全部收口**(线上 `d16620f`/端口 3013)。**「P1 功能全部做到」这个说法曾被 2026-08-08 全量复核推翻**——计费/对账/状态机/安全那一层是真做到了,但生成器面板比 CHECKLIST 承诺的薄(60 项 = implemented 15 / partial 30 / missing 15,R2-Q4)。**2026-08-09 用户裁决走混合路线:9 项改判「延 P2」(P1 由 61 降为 52),#185 拦截式确认按原规格改代码,余下缺口按「便宜活优先」分批补齐。**画布在生产以 canary 灰度运行(白名单 2 人,视频模型 `happyhorse`)。当前在做的事:**R2-Q4 补齐批次**。**扩灰度须等补齐批次过闸。P2 尚未开工,未经用户裁决不要自行启动。**
...
| P1 生成接入 | 52 | ⚠️ R1/R2 均已收口并上线;**2026-08-09 用户裁决把 9 项改判「延 P2」后从 61 降为 52**(R2-Q4);剩余缺口=补齐批次,见 P0 看板 |
| P2 | 55 | 未开工,无看板 |
| P3 / P4 | 11 / 1 | 未开工 |
...
- 线上版本 **`d16620f34f09…`(2026-08-08 二次蓝绿发布,端口 3013,`DEPLOY_RC=0`)**;上一版 `abc29ac…`/3012 仍在线待回滚,**别把它当成当前线上**。12 个画布迁移已在生产执行(第 12 个是 2026-08-08 的图片 unknown 恢复)
- 2026-08-09 复核第零步全绿:站点 200、`/canvas` 匿名 307、nginx→3013、两个 `CANVAS_VIDEO_MODELS=happyhorse`、`stargaze-canvas-d16620f` 与 reconciler 均 online、磁盘余 12G
...
4. 🔴 **下一步=蓝绿发版**(手工 build 必须带 `CANVAS_RELEASE_COMMIT=<40位sha>`,否则 BUILD_ID 门 FAIL;再发 2 次前先清最老的 release,现磁盘余 12G),发版后做一轮生产 UI 复验,**再**谈扩灰度(白名单 2→3-5 观察 3-7 天→`CANVAS_PUBLIC_ENABLED=true`)
